...
@Slf4j
@Component
@Tag(name = "轮班制计算策略", description = "三班倒、四班三倒等轮班模式工时计算策略")
public class RotatingWorkTimeStrategy implements WorkTimeCalculateStrategy {

    /**
     * 夜班津贴系数（1.2倍）
     */
    private static final Double NIGHT_SHIFT_ALLOWANCE_RATE = 1.2;

    /**
     * 中班津贴系数（1.1倍）
     */
    private static final Double EVENING_SHIFT_ALLOWANCE_RATE = 1.1;

    @Override
    public String getStrategyType() {
        return WorkTimeCalculateStrategyFactory.STRATEGY_ROTATING;
    }
    ...
    public CalculateResult calculate(CalculateContext context) {
        log.info("[轮班制策略] 开始计算工时: employeeId={}, attendanceDate={}, shiftName={}",
        ...
        try {
            ...
            // 9. 计算津贴系数（夜班/中班）
            Double allowanceRate = calculateAllowanceRate(context.getWorkShift());
            ...
            log.error("[轮班制策略] 计算异常: employeeId={}, error={}",
    ...
    public boolean validatePunchRecords(List<PunchRecord> punchRecords, WorkShiftEntity workShift) {
        ...
        if (!isValid) {
            log.warn("[轮班制策略] 打卡记录验证失败: hasClockIn={}, hasClockOut={}, allValid={}",
    ...
    public Integer calculateLateMinutes(LocalDateTime punchTime, WorkShiftEntity workShift) {
        ...
        // 如果是夜班（00:00-08:00），上班时间可能在第二天
        LocalDate startDate = punchDate;
        if (isNightShift(workShift) && punchTime.getHour() >= 0 && punchTime.getHour() < 12) {
            // 夜班上班时间在当天
            startDate = punchDate;
        } else if (isNightShift(workShift)) {
            // 夜班上班时间在第二天
            startDate = punchDate.minusDays(1);
        }

        LocalDateTime standardStartTime = LocalDateTime.of(startDate, workShift.getWorkStartTime());
        ...
        log.debug("[轮班制策略] 计算迟到: punchTime={}, standardTime={}, lateMinutes={}",
    ...
    public Integer calculateEarlyLeaveMinutes(LocalDateTime punchTime, WorkShiftEntity workShift) {
        ...
        log.debug("[轮班制策略] 计算早退: punchTime={}, standardTime={}, earlyLeaveMinutes={}",
    ...
    public Integer calculateOvertimeMinutes(LocalDateTime endTime, WorkShiftEntity workShift) {
        ...
        if (minOvertimeMinutes != null && overtimeMinutes < minOvertimeMinutes) {
            log.debug("[轮班制策略] 加班时长不足最小要求: overtimeMinutes={}, minOvertimeMinutes={}",
                    overtimeMinutes, minOvertimeMinutes);
            return 0;
        }

        log.debug("[轮班制策略] 计算加班: endTime={}, overtimeStartTime={}, overtimeMinutes={}",
    ...
    public List<WorkTimeSpan> calculateWorkTimeSpans(List<PunchRecord> punchRecords, WorkShiftEntity workShift) {
        ...
        for (int i = 0; i < sortedRecords.size(); i++) {
            ...
            if (record.getPunchType() == PunchType.IN) {
                ...
                // 判断是否为跨天时段
                boolean isOvernightSpan = record.getPunchTime().toLocalDate()
                        .isAfter(startTime.toLocalDate());

                // 构建工作时段
                int workMinutes = (int) java.time.Duration.between(startTime, record.getPunchTime()).toMinutes();

                // 根据时间段判断津贴类型
                TimeSpanType spanType = determineTimeSpanType(startTime, workShift);
    ...
    public WorkTimeSpan getAllowedPunchTimeRange(WorkShiftEntity workShift) {
        ...
        // 轮班制允许更灵活的打卡时间范围
        LocalDateTime earlyPunchTime = LocalDateTime.of(today, workShift.getWorkStartTime()).minusHours(2);
        LocalDate endDate = isOvernightShift(workShift) ? today.plusDays(1) : today;
        LocalDateTime latePunchTime = LocalDateTime.of(endDate, workShift.getWorkEndTime()).plusHours(2);
     ...
     * @return 是否为夜班
     */
    private boolean isNightShift(WorkShiftEntity workShift) {
        if (workShift == null || workShift.getWorkStartTime() == null || workShift.getWorkEndTime() == null) {
            return false;
        }

        // 夜班判断：开始时间在0:00-8:00之间
        int startHour = workShift.getWorkStartTime().getHour();
        int endHour = workShift.getWorkEndTime().getHour();

        // 例如：00:00-08:00 或 22:00-06:00
        return (startHour >= 0 && startHour < 8) || (endHour >= 0 && endHour < 8);
    }
     ...
     * @return 津贴系数
     */
    private Double calculateAllowanceRate(WorkShiftEntity workShift) {
        if (workShift == null || workShift.getShiftType() == null) {
            return 1.0;
        }
        ...
        return switch (shiftType) {
            case NIGHT_SHIFT -> NIGHT_SHIFT_ALLOWANCE_RATE;
            case ROTATING_SHIFT -> {
                // 轮班制根据时间段判断
                if (isNightShift(workShift)) {
                    yield NIGHT_SHIFT_ALLOWANCE_RATE;
                } else if (isEveningShift(workShift)) {
                    yield EVENING_SHIFT_ALLOWANCE_RATE;
                } else {
                    yield 1.0;
                }
            }
     ...
     * @return 是否为中班
     */
    private boolean isEveningShift(WorkShiftEntity workShift) {
        if (workShift == null || workShift.getWorkStartTime() == null) {
            return false;
        }
        // 中班判断：开始时间在16:00-24:00之间
        int startHour = workShift.getWorkStartTime().getHour();
        return startHour >= 16 && startHour < 24;
    }
     ...
     * @return 时段类型
     */
    private TimeSpanType determineTimeSpanType(LocalDateTime startTime, WorkShiftEntity workShift) {
        if (isNightShift(workShift) || (startTime.getHour() >= 22 || startTime.getHour() < 6)) {
            return TimeSpanType.OVERTIME; // 夜班时段
        } else if (startTime.getHour() >= 16 && startTime.getHour() < 24) {
            return TimeSpanType.FLEXIBLE; // 中班时段
        } else {
            return TimeSpanType.NORMAL; // 正常时段
        }
    }
