...
public class ScheduleServiceImpl implements ScheduleService {
    ...
    public void batchCreateScheduleRecords (List<ScheduleRecordForm> forms) {
        ...
        // 3. 转换并批量插入
        List<ScheduleRecordEntity> records = forms.stream ().map (form -> {
            ScheduleRecordEntity record = SmartBeanUtil.copy (form, ScheduleRecordEntity.class);
            record.setScheduleType ("正常排班");
            record.setIsTemporary (false);
            record.setStatus (ScheduleRecordEntity.ScheduleStatus.NORMAL.getCode ());
    ...
    public void updateScheduleRecord (Long scheduleId, ScheduleRecordForm form) {
        ...
        // 更新记录
        SmartBeanUtil.copyProperties (form, record);
    ...
    public List<ScheduleRecordVO> getEmployeeSchedules (Long employeeId, LocalDate startDate, LocalDate endDate) {
        ...
        List<ScheduleRecordEntity> records = scheduleRecordDao.selectByEmployeeIdAndDateRange (employeeId, startDate,
                endDate);

        return records.stream ().map (this::convertToScheduleRecordVO).collect (Collectors.toList ());
    }
    ...
    public List<ScheduleRecordVO> getDepartmentSchedules (Long departmentId, LocalDate startDate, LocalDate endDate) {
        ...
        List<ScheduleRecordEntity> records = scheduleRecordDao.selectByDepartmentAndDateRange (departmentId, startDate,
                endDate);
    ...
    public void updateScheduleTemplate (Long templateId, ScheduleTemplateForm form) {
        ...
        SmartBeanUtil.copyProperties (form, template);
    ...
    public List<ScheduleTemplateVO> getScheduleTemplates (String templateType, Long departmentId) {
        ...
        return templates.stream ().map (this::convertToScheduleTemplateVO).collect (Collectors.toList ());
    }
    ...
    public SchedulingResultVO generateSmartSchedule (SmartSchedulingForm form) {
        ...
        request.setPeriod (
                form.getStartDate ().format (DATE_FORMATTER) + "~" + form.getEndDate ().format (DATE_FORMATTER));
    ...
    public SchedulingStatisticsVO getSchedulingStatistics (Long departmentId, LocalDate startDate, LocalDate endDate) {
        ...
        java.util.Map<String, Object> stats = scheduleRecordDao.selectDepartmentStats (departmentId, startDate,
                endDate);

        SchedulingStatisticsVO statistics = new SchedulingStatisticsVO ();
        statistics.setDepartmentId (departmentId);
        statistics.setPeriod (startDate.format (DATE_FORMATTER) + "~" + endDate.format (DATE_FORMATTER));
    ...
    public ScheduleResult executeIntelligentSchedule (ScheduleRequest request) {
        return scheduleEngine.executeIntelligentSchedule (request);
    }

    @Override
    public SchedulePlan generateSchedulePlan (Long planId, LocalDate startDate, LocalDate endDate) {
        return scheduleEngine.generateSchedulePlan (planId, startDate, endDate);
    }

    @Override
    public ConflictDetectionResult validateScheduleConflicts (ScheduleData scheduleData) {
        return scheduleEngine.validateScheduleConflicts (scheduleData);
    }

    @Override
    public ConflictResolution resolveScheduleConflicts (List<ScheduleConflict> conflicts, String strategy) {
        return scheduleEngine.resolveScheduleConflicts (conflicts, strategy);
    }

    @Override
    public OptimizedSchedule optimizeSchedule (ScheduleData scheduleData, String optimizationTarget) {
        return scheduleEngine.optimizeSchedule (scheduleData, optimizationTarget);
    }

    @Override
    public ScheduleStatistics getScheduleStatistics (Long planId) {
        return scheduleEngine.getScheduleStatistics (planId);
    }

    @Override
    public SchedulePrediction predictScheduleDemand (Long departmentId, LocalDate startDate, LocalDate endDate) {
        int timeRange = 0;
        if (startDate != null && endDate != null) {
            timeRange = Math.max (0, (int) (endDate.toEpochDay () - startDate.toEpochDay () + 1));
        }

        return SchedulePrediction.builder ().predictionId (UUID.randomUUID ().toString ())
                .predictionType ("SCHEDULE_DEMAND")
                .timeRange (SchedulePrediction.TimeRange.builder ()
                        .startDate (startDate != null ? startDate.toString () : "")
                        .endDate (endDate != null ? endDate.toString () : "").duration (timeRange).timeUnit ("DAYS")
                        .build ())
                .predictionTime (LocalDateTime.now ()).success (true).predictionMessage ("排班需求预测完成").build ();
    }

    @Override
    public SchedulePrediction predictScheduleEffect (ScheduleData scheduleData) {
        return scheduleEngine.predictScheduleEffect (scheduleData);
    }
    ...
    private ScheduleRecordVO convertToScheduleRecordVO (ScheduleRecordEntity entity) {
        ...
        if (entity.getStatus () != null) {
            ScheduleRecordEntity.ScheduleStatus status = ScheduleRecordEntity.ScheduleStatus
                    .fromCode (entity.getStatus ());
    ...
    private ScheduleTemplateVO convertToScheduleTemplateVO (ScheduleTemplateEntity entity) {
        ...
        if (entity.getTemplateType () != null) {
            ScheduleTemplateEntity.TemplateType type = ScheduleTemplateEntity.TemplateType
                    .fromDescription (entity.getTemplateType ());
            vo.setTemplateTypeDesc (type.getDescription ());
        }

        if (entity.getStatus () != null) {
            ScheduleTemplateEntity.TemplateStatus status = ScheduleTemplateEntity.TemplateStatus
                    .fromCode (entity.getStatus ());
            vo.setStatusDesc (status.getDescription ());
        }
    ...
    private SchedulingResultVO convertToSchedulingResultVO (SchedulingResult result) {
        ...
        if (result.getScheduleRecords () != null) {
            vo.setScheduleRecords (result.getScheduleRecords ().stream ().map (this::convertToScheduleRecordVO)
                    .collect (Collectors.toList ()));
        }
     ...
     * 转换统计信息为VO
     */
    private SchedulingStatisticsVO convertToSchedulingStatisticsVO (
            SmartSchedulingEngine.SchedulingStatistics statistics) {
        if (statistics == null) {
