package net.lab1024.sa.attendance.domain.service;
...
public class AttendanceSyncService {
    ...
    public UploadSyncResult uploadSingleRecord(Long employeeId, MobileSyncData syncData) {
        try {
            ...
            // 4. 保存考勤记录（使用 insert 方法）
            int result = attendanceRecordDao.insert(record);
    ...
    public DownloadSyncResult downloadEmployeeData(Long employeeId, LocalDateTime lastSyncTime) {
        try {
            ...
            // 1. 查询增量数据（使用日期范围查询）
            List<AttendanceRecordEntity> records = attendanceRecordDao.selectByDateRange(employeeId,
    ...
    private AttendanceRecordEntity findExistingRecord(Long employeeId, MobileSyncData syncData) {
        ...
        List<AttendanceRecordEntity> records = attendanceRecordDao.selectTodayRecord(employeeId,
                syncData.getPunchTime());
    ...
    private AttendanceRecordEntity convertSyncDataToRecord(Long employeeId, MobileSyncData syncData) {
        ...
        record.setAttendanceDate(syncData.getPunchTime().toLocalDate());

        // 注意：以下字段在 AttendanceRecordEntity 中可能不存在，这里先注释掉
        ...
        // record.setGpsValidation(syncData.getGpsValidation() != null &&
        // syncData.getGpsValidation() == 1);
    ...
    private MobileSyncData convertRecordToSyncData(AttendanceRecordEntity record) {
        ...
        data.setPunchTime(record.getPunchInTime() != null
                ? record.getAttendanceDate().atTime(record.getPunchInTime())
                : record.getAttendanceDate().atTime(record.getPunchOutTime()));
        data.setPunchType(record.getPunchInTime() != null ? "IN" : "OUT");
        // 注意：以下字段在 AttendanceRecordEntity 中可能不存在，这里先设置为null
