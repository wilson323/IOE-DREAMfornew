...
public class AttendanceManager {
    ...
    public UserDetailVO getUserDetail(final Long userId) {
        if (userId == null) {
        ...
        try {
            final ResponseDTO<UserDetailVO> response = gatewayClient.callCommonService(
                    "/api/v1/users/" + userId,
                    HttpMethod.GET,
                    null,
                    UserDetailVO.class
            ...
            if (response != null && response.isSuccess()) {
                final UserDetailVO data = response.getData();
                if (data != null) {
    ...
    public String getUserName(final Long userId) {
        final UserDetailVO userDetail = getUserDetail(userId);
        if (userDetail == null) {
            return UNKNOWN_USER;
        }
     ...
     */
    @SuppressWarnings("null")
    public boolean updateAttendanceStatistics(final Long userId, final LocalDate date,
            final String attendanceType, final BigDecimal value) {
        ...
        try {
            ...
            final ResponseDTO<Void> response = gatewayClient.callAttendanceService(
            ...
            if (response != null && response.isSuccess()) {
                ...
                if (log.isErrorEnabled()) {
                    log.error("[考勤管理] 更新考勤统计失败，userId={}, date={}, attendanceType={}, value={}, message={}",
                            userId, date, attendanceType, value, message);
                }
            }
        } catch (RuntimeException e) {
            if (log.isErrorEnabled()) {
     ...
     * @return 是否处理成功
     */
    public boolean processLeaveApproval(final Long userId, final String leaveType,
            final LocalDate startDate, final LocalDate endDate, final Double leaveDays) {
        if (log.isInfoEnabled()) {
        ...
        while (!currentDate.isAfter(endDate)) {
            final boolean updateSuccess = updateAttendanceStatistics(
                    userId,
                    currentDate,
                    "LEAVE",
     ...
     */
    @SuppressWarnings("null")
    public boolean processShiftApproval(final Long userId, final LocalDate shiftDate,
            final Long originalShiftId, final Long targetShiftId) {
        if (log.isInfoEnabled()) {
    ...
    public boolean processSupplementApproval(final Long userId, final LocalDate supplementDate,
        ...
        try {
            ...
            if (response != null && response.isSuccess()) {
                ...
                final String message = response != null ? response.getMessage() : RESPONSE_NULL_MSG;
                if (log.isErrorEnabled()) {
                    log.error("[考勤管理] 补签审批通过处理失败，userId={}, supplementDate={}, message={}", userId, supplementDate, message);
                }
            }
        } catch (RuntimeException e) {
            if (log.isErrorEnabled()) {
     ...
     */
    @SuppressWarnings("null")
    public boolean processOvertimeApproval(final Long userId, final LocalDate overtimeDate, final Double overtimeHours) {
        if (log.isInfoEnabled()) {
        ...
        try {
            ...
            if (response != null && response.isSuccess()) {
                ...
                final String message = response != null ? response.getMessage() : RESPONSE_NULL_MSG;
                if (log.isErrorEnabled()) {
                    log.error("[考勤管理] 加班审批通过处理失败，userId={}, overtimeDate={}, message={}", userId, overtimeDate, message);
                }
            }
        } catch (RuntimeException e) {
            if (log.isErrorEnabled()) {
     ...
     */
    @SuppressWarnings("null")
    public boolean processTravelApproval(final Long userId, final LocalDate travelStartDate, final LocalDate travelEndDate) {
        if (log.isInfoEnabled()) {
        ...
        try {
            ...
            if (response != null && response.isSuccess()) {
                ...
                final String message = response != null ? response.getMessage() : RESPONSE_NULL_MSG;
                if (log.isErrorEnabled()) {
                    log.error("[考勤管理] 出差审批通过处理失败，userId={}, travelStartDate={}, message={}", userId, travelStartDate, message);
                }
            }
        } catch (RuntimeException e) {
            if (log.isErrorEnabled()) {
