...
import net.lab1024.sa.attendance.domain.vo.CustomReportResult;
import net.lab1024.sa.attendance.domain.vo.ExportResult;
import net.lab1024.sa.attendance.domain.vo.PivotTableResult;
import net.lab1024.sa.attendance.domain.vo.ReportConfigValidationResult;
...
public class AttendanceCustomReportService {
     ...
     * 根据配置处理数据
     */
    private List<Map<String, Object>> processDataByConfig(List<Map<String, Object>> rawData,
    ...
    private CustomReportResult buildReportResult(CustomReportConfig config, List<Map<String, Object>> data) {
        ...
        result.setData(data);
        result.setColumns(convertColumnInfoToReportColumn(buildColumnInfo(config)));
    ...
    /**
     * 将ColumnInfo转换为ReportColumn
     */
    private List<CustomReportResult.ReportColumn> convertColumnInfoToReportColumn(List<ColumnInfo> columnInfos) {
        List<CustomReportResult.ReportColumn> reportColumns = new ArrayList<>();
        for (ColumnInfo columnInfo : columnInfos) {
            CustomReportResult.ReportColumn reportColumn = new CustomReportResult.ReportColumn();
            reportColumn.setField(columnInfo.getFieldName());
            reportColumn.setTitle(columnInfo.getDisplayName());
            reportColumn.setDataType(columnInfo.getDataType());
            reportColumn.setVisible(true);
            reportColumn.setSortable(true);
