package net.lab1024.sa.attendance.engine.rule.model;

import lombok.Data;
import lombok.Builder;
import lombok.AllArgsConstructor;
import lombok.NoArgsConstructor;

import java.time.LocalDateTime;
import java.util.Map;

/**
 * 编译后的规则动作模型
 * <p>
 * 规则动作配置编译后的可执行对象
 * 严格遵循CLAUDE.md全局架构规范
 * </p>
 *
 * @author IOE-DREAM架构团队
 * @version 1.0.0
 * @since 2025-12-16
 */
@Data
@Builder
@NoArgsConstructor
@AllArgsConstructor
public class CompiledAction {

    /**
     * 动作ID
     */
    private String actionId;

    /**
     * 动作类型
     */
    private String actionType;

    /**
     * 动作配置
     */
    private String actionConfiguration;

    /**
     * 编译后的动作对象
     */
    private Object compiledAction;

    /**
     * 是否已编译
     */
    private Boolean compiled;

    /**
     * 编译时间
     */
    private LocalDateTime compileTime;

    /**
     * 编译耗时（毫秒）
     */
    private Long compileDuration;

    /**
     * 编译错误信息
     */
    private String compileError;

    /**
     * 编译警告信息
     */
    private java.util.List<String> compileWarnings;

    /**
     * 编译统计信息
     */
    private Map<String, Object> compileStatistics;

    /**
     * 编译器版本
     */
    private String compilerVersion;

    /**
     * 编译器名称
     */
    private String compilerName;

    /**
     * 优化级别
     */
    private String optimizationLevel;

    /**
     * 是否需要重新编译
     */
    private Boolean needsRecompile;

    /**
     * 动作参数定义
     */
    private Map<String, Object> parameterDefinitions;

    /**
     * 动作变量映射
     */
    private Map<String, String> variableMapping;

    /**
     * 执行优先级
     */
    private Integer executionPriority;

    /**
     * 是否关键动作
     */
    private Boolean isCritical;

    /**
     * 超时时间（毫秒）
     */
    private Long timeoutMillis;

    /**
     * 重试次数
     */
    private Integer retryCount;

    /**
     * 回退策略
     */
    private String fallbackStrategy;

    /**
     * 创建成功的编译结果
     */
    public static CompiledAction success(String actionId, String actionType, String actionConfiguration, Object compiledAction) {
        return CompiledAction.builder()
                .actionId(actionId)
                .actionType(actionType)
                .actionConfiguration(actionConfiguration)
                .compiledAction(compiledAction)
                .compiled(true)
                .compileTime(LocalDateTime.now())
                .compilerName("DefaultActionCompiler")
                .compilerVersion("1.0.0")
                .needsRecompile(false)
                .executionPriority(0)
                .isCritical(false)
                .retryCount(0)
                .build();
    }

    /**
     * 创建失败的编译结果
     */
    public static CompiledAction failure(String actionId, String actionType, String actionConfiguration, String compileError) {
        return CompiledAction.builder()
                .actionId(actionId)
                .actionType(actionType)
                .actionConfiguration(actionConfiguration)
                .compiled(false)
                .compileError(compileError)
                .compileTime(LocalDateTime.now())
                .compilerName("DefaultActionCompiler")
                .compilerVersion("1.0.0")
                .needsRecompile(false)
                .build();
    }

    /**
     * 检查编译是否成功
     */
    public boolean isCompileSuccess() {
        return Boolean.TRUE.equals(compiled) && compiledAction != null;
    }

    /**
     * 检查是否有编译错误
     */
    public boolean hasCompileError() {
        return !Boolean.TRUE.equals(compiled) && compileError != null;
    }

    /**
     * 检查是否为关键动作
     */
    public boolean isCriticalAction() {
        return Boolean.TRUE.equals(isCritical);
    }

    /**
     * 检查是否需要重试
     */
    public boolean needsRetry() {
        return retryCount != null && retryCount > 0;
    }

    /**
     * 添加编译警告
     */
    public void addCompileWarning(String warning) {
        if (this.compileWarnings == null) {
            this.compileWarnings = new java.util.ArrayList<>();
        }
        this.compileWarnings.add(warning);
    }

    /**
     * 设置编译统计信息
     */
    public void setCompileStatistic(String key, Object value) {
        if (this.compileStatistics == null) {
            this.compileStatistics = new java.util.HashMap<>();
        }
        this.compileStatistics.put(key, value);
    }

    /**
     * 设置参数定义
     */
    public void setParameterDefinition(String paramName, Object definition) {
        if (this.parameterDefinitions == null) {
            this.parameterDefinitions = new java.util.HashMap<>();
        }
        this.parameterDefinitions.put(paramName, definition);
    }

    /**
     * 设置变量映射
     */
    public void setVariableMapping(String variableName, String mapping) {
        if (this.variableMapping == null) {
            this.variableMapping = new java.util.HashMap<>();
        }
        this.variableMapping.put(variableName, mapping);
    }

    /**
     * 获取编译结果摘要
     */
    public String getCompileSummary() {
        if (isCompileSuccess()) {
            return String.format("编译成功 - 动作: %s, 类型: %s, 耗时: %dms", actionId, actionType, compileDuration);
        } else if (hasCompileError()) {
            return String.format("编译失败 - 动作: %s, 类型: %s, 错误: %s", actionId, actionType, compileError);
        } else {
            return String.format("编译状态未知 - 动作: %s, 类型: %s", actionId, actionType);
        }
    }

    /**
     * 获取执行配置摘要
     */
    public String getExecutionSummary() {
        return String.format(
            "执行配置 - 优先级: %d, 超时: %dms, 重试: %d, 关键: %s",
            executionPriority != null ? executionPriority : 0,
            timeoutMillis != null ? timeoutMillis : 0,
            retryCount != null ? retryCount : 0,
            isCriticalAction()
        );
    }
}