...
public class BacktrackAlgorithmImpl implements ScheduleAlgorithm {
    ...
    public ScheduleResult generateSchedule(ScheduleData scheduleData) {
        ...
        try {
            ...
            if (found && state.getCurrentSolution() != null) {
                ...
                List<ScheduleResult.ScheduleRecord> resultRecords = scheduleRecords.stream()
                        .map(this::convertToResultScheduleRecord)
                        .collect(Collectors.toList());

                Map<String, Object> metrics = new HashMap<>();
                metrics.put("totalBacktracks", totalBacktracks.get());
                metrics.put("successfulAssignments", successfulAssignments.get());
                metrics.put("failedAttempts", failedAttempts.get());
                metrics.put("pruningStrategy", pruningStrategy);
    ...
    private boolean backtrack(BacktrackState state, int depth, int maxDepth, long timeLimit) {
        ...
        // 检查时间限制
        if (System.currentTimeMillis() - state.startTime > timeLimit) {
    ...
    public AlgorithmComplexity getComplexity() {
        AlgorithmComplexity complexity = new AlgorithmComplexity("O(b^n)", "O(n)");
        complexity.setWorstCaseTime("O(b^n)");
        complexity.setBestCaseTime("O(n)");
        complexity.setAverageCaseTime("O(b^n)");
        return complexity;
    }
    ...
    public AlgorithmMetadata getMetadata() {
        return AlgorithmMetadata.builder()
                .name("回溯算法")
                .version("1.0.0")
                .description("基于回溯策略的排班算法，通过深度优先搜索找到精确解")
                .author("IOE-DREAM架构团队")
                ...
                .parameters(java.util.Arrays.asList(
                        ...
                        AlgorithmMetadata.AlgorithmParameter.builder()
                                ...
                                .displayName("剪枝策略")
                                .type("STRING")
                                .defaultValue("FORWARD_CHECKING")
                                .description("剪枝策略：FORWARD_CHECKING, CONSISTENCY_CHECKING, MIN_REMAINING_VALUES")
                                .required(false)
                                .build()))
                .build();
    }
