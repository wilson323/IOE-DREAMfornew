package net.lab1024.sa.attendance.dao.tool;
...
public class AttendancePerformanceAnalyzer {
    ...
    public Map<String, Object> analyzeTableStatistics() {
        ...
        try {
            ...
            tableStats.put("overallIndexRatio",
                    Math.round(totalIndexSize / (totalSize + totalIndexSize) * 10000.0) / 100.0);
    ...
    private List<Map<String, Object>> analyzeHighFrequencyQueries() {
        ...
        Map<String, Object> query2 = new LinkedHashMap<>();
        query2.put("query",
                "SELECT COUNT(*) FROM t_attendance_record WHERE attendance_date >= ? AND attendance_date <= ?");
    ...
    public Map<String, Object> executePerformanceBenchmark() {
        ...
        try {
            ...
            benchmark.put("averageQueryTime",
                    queryTimes.values().stream().mapToLong(Long::longValue).average().orElse(0.0));
