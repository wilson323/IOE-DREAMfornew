...
public class AttendanceReportService {
    ...
    public EmployeeDailyReport generateEmployeeDailyReport(Long employeeId, LocalDate date) {
        try {
            ...
            // 1. 查询当日考勤记录
            List<AttendanceRecordEntity> records = attendanceRepository.selectTodayRecord(employeeId,
                    date.atStartOfDay());
            AttendanceRecordEntity record = records != null && !records.isEmpty() ? records.get(0) : null;

            // 2. 获取员工适用的考勤规则（需要4个参数：employeeId, departmentId, employeeType, queryDate）
    ...
    public DepartmentDailyReport generateDepartmentDailyReport(Long departmentId, LocalDate date) {
        try {
            ...
            // 1. 查询部门所有员工的当日考勤记录（使用日期范围查询）
            LocalDateTime startDateTime = date.atStartOfDay();
            LocalDateTime endDateTime = date.atTime(23, 59, 59);
            List<AttendanceRecordEntity> records = attendanceRepository.selectByDateRange(null,
    ...
    public EmployeeMonthlyReport generateEmployeeMonthlyReport(Long employeeId, String yearMonth) {
        try {
            ...
            List<AttendanceRecordEntity> records = attendanceRepository.selectByDateRange(employeeId,
                    startDateTime, endDateTime);
    ...
    public DepartmentMonthlyReport generateDepartmentMonthlyReport(Long departmentId, String yearMonth) {
        try {
            ...
            // 2. 查询部门月度考勤记录（使用日期范围查询）
            LocalDateTime startDateTime = startDate.atStartOfDay();
            LocalDateTime endDateTime = endDate.atTime(23, 59, 59);
            List<AttendanceRecordEntity> allRecords = attendanceRepository.selectByDateRange(null,
                    startDateTime, endDateTime);
            // 过滤出指定部门的记录
            List<AttendanceRecordEntity> records = allRecords.stream()
                    .filter(r -> departmentId.equals(r.getDepartmentId()))
                    .collect(Collectors.toList());
    ...
    public ExceptionStatisticsReport generateExceptionStatisticsReport(Long employeeId, LocalDate startDate,
        ...
        try {
            ...
            // 1. 查询异常记录（使用日期范围查询，然后过滤异常记录）
            LocalDateTime startDateTime = startDate.atStartOfDay();
            LocalDateTime endDateTime = endDate.atTime(23, 59, 59);
            List<AttendanceRecordEntity> allRecords = attendanceRepository.selectByDateRange(employeeId,
                    startDateTime, endDateTime);
            // 过滤出有异常的记录
            List<AttendanceRecordEntity> records = allRecords.stream()
    ...
    private void fillDailyReportFromRecord(EmployeeDailyReport report, AttendanceRecordEntity record) {
        ...
        // GPS和设备验证状态：AttendanceRecordEntity中没有这些字段，设置为null
        // 如果需要，可以从考勤规则中获取验证要求
        report.setGpsValidation(null);
        report.setDeviceValidation(null);
    ...
    private void fillDailyReportWithRules(EmployeeDailyReport report, List<AttendanceRuleEntity> rules) {
        if (rules != null && !rules.isEmpty()) {
            ...
            // 获取LocalTime类型的工作时间
            java.time.LocalTime workStartTime = primaryRule.getWorkStartTime();
            java.time.LocalTime workEndTime = primaryRule.getWorkEndTime();
    ...
    private void fillTrendAnalysisFromRecords(AttendanceTrendAnalysis analysis, List<AttendanceRecordEntity> records) {
        // 按日期分组统计（getAttendanceDate() 已经返回 LocalDate，不需要转换）
