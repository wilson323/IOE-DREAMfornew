...
public class AttendanceScheduleServiceImpl extends ServiceImpl<AttendanceScheduleDao, AttendanceScheduleEntity>
    ...
    @Resource
    private net.lab1024.sa.common.hr.dao.EmployeeDao employeeDao;
    ...
    public List<AttendanceScheduleEntity> getDepartmentSchedule(Long departmentId, LocalDate startDate,
        ...
        try {
            ...
            employeeWrapper.eq(EmployeeEntity::getDepartmentId, departmentId)
                    .eq(EmployeeEntity::getStatus, 1)
                    .eq(EmployeeEntity::getDeletedFlag, 0);

            List<EmployeeEntity> employees = employeeDao.selectList(employeeWrapper);
    ...
    private boolean hasScheduleConflict(AttendanceScheduleEntity schedule) {
        try {
            ...
            if (schedule.getShiftId() != null) {
                ...
                // attendanceShiftDao.selectById(schedule.getShiftId());
                // if (shift == null || shift.getStatus() == null ||
                // !"ACTIVE".equals(shift.getStatus())) {
    ...
    public String validateSchedule(AttendanceScheduleEntity schedule) {
        try {
            ...
            // 验证员工是否存在且有效
            EmployeeEntity employee = employeeDao.selectById(schedule.getEmployeeId());
            if (employee == null) {
                return "员工不存在";
            }
    ...
    public PageResult<AttendanceScheduleEntity> querySchedulePage(PageParam pageParam, Long employeeId,
        ...
        try {
            ...
            if (departmentId != null) {
                // 查询部门下的所有员工ID
                LambdaQueryWrapper<EmployeeEntity> departmentEmployeeWrapper = new LambdaQueryWrapper<>();
                departmentEmployeeWrapper.eq(EmployeeEntity::getDepartmentId, departmentId)
                        .eq(EmployeeEntity::getStatus, 1)
                        .eq(EmployeeEntity::getDeletedFlag, 0);

                List<EmployeeEntity> employees = employeeDao.selectList(departmentEmployeeWrapper);
                if (employees == null || employees.isEmpty()) {
                    // 部门下无员工，返回空结果
            ...
            return PageResult.of(List.of(), 0L, pageParam.getPageNum(), pageParam.getPageSize());
        }
    }
