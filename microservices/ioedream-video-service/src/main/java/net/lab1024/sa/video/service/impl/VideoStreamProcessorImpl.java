...
import net.lab1024.sa.video.service.VideoStreamProcessor;
...
public class VideoStreamProcessorImpl implements VideoStreamProcessor {
    ...
    public boolean startRtspStream(Long deviceId, String rtspUrl, String streamId) {
        try {
            ...
            // 缓存流状态
            cacheService.set("video:stream:" + streamId, streamInfo, 3600L, TimeUnit.SECONDS);
    ...
    public Map<String, Object> getStreamStatus(String streamId) {
        try {
            ...
            if (streamInfo == null) {
                // 尝试从缓存获取
                Object cachedObj = cacheService.get("video:stream:" + streamId);
                if (cachedObj instanceof StreamInfo) {
                    streamInfo = (StreamInfo) cachedObj;
                }
                if (streamInfo == null) {
