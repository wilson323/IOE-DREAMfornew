...
public class WorkflowApprovalManager {
    ...
    // 已移除：审批配置缓存（已迁移到Service层使用@Cacheable注解）
    // 缓存逻辑应在Service层使用@Cacheable注解处理

    // 已移除：重试配置（已迁移到Resilience4j标准方案）
    // 重试应在Service层使用@Retry注解实现
    ...
    public ResponseDTO<Long> startApprovalProcess(
        ...
        try {
            Long resolvedDefinitionId = definitionId;

            // 1. 获取流程定义ID（支持缓存优化）
            if (resolvedDefinitionId == null) {
                resolvedDefinitionId = getDefinitionIdWithCache(businessType);
                if (resolvedDefinitionId == null) {
                ...
                log.info("从审批配置中获取流程定义ID，业务类型: {}, definitionId: {}", businessType, resolvedDefinitionId);
            }
            ...
            Map<String, Object> requestParams = buildRequestParams(
                resolvedDefinitionId, businessKey, instanceName, flowVariables, formData);

            // 5. 执行远程调用
            // 注意：重试逻辑应在Service层使用Resilience4j的@Retry注解实现
            // 参考：documentation/technical/RESILIENCE4J_MIGRATION_GUIDE.md
            ResponseDTO<Long> result = executeRemoteCall(
     ...
     * 获取流程定义ID
     * <p>
     * 注意：缓存逻辑应在Service层使用@Cacheable注解处理
     * </p>
     */
    private Long getDefinitionIdWithCache(String businessType) {
        ...
        // 直接查询（缓存由Service层@Cacheable注解处理）
        try {
            Long definitionId = approvalConfigManager.getDefinitionId(businessType);
            log.debug("获取审批配置，业务类型: {}, definitionId: {}", businessType, definitionId);
            return definitionId;
        } catch (Exception e) {
     ...
     * 执行远程调用（已移除重试逻辑）
     * <p>
     * ✅ 已迁移到Resilience4j标准方案
     * - 已移除自定义重试逻辑
     * - 重试应在Service层使用@Retry注解实现
     * - 参考 {@link documentation/technical/RESILIENCE4J_MIGRATION_GUIDE.md} 迁移指南
     * </p>
     * <p>
     * 使用说明：
     * - 在调用startApprovalProcess的Service层方法上使用@Retry注解
     * - 配置Resilience4j的重试策略（指数退避、最大重试次数等）
     * - 参考 {@link documentation/technical/RESILIENCE4J_USAGE_GUIDE.md} 使用指南
     * </p>
     */
    private <T> ResponseDTO<T> executeRemoteCall(String apiPath, Map<String, Object> params,
                                                  Class<T> responseType, String operationName) {
        log.debug("执行远程调用，操作: {}", operationName);
