 ...
 * JSON Schema 表单引擎服务
 * <p>
 * Flowable 7.2.0已移除内置Form引擎，本服务使用JSON Schema替代， 提供企业级表单管理功能，支持动态表单定义、表单实例管理、 表单验证、表单渲染、表单版本控制等功能
 * </p>
 *
...
@Slf4j
 public class FormEngineService {
     ...
     * @param formSchema
     *            JSON Schema格式的表单定义
     * @param formDefinitionKey
     *            表单定义键
     * @param formDefinitionName
     *            表单定义名称
     * @return 部署结果（表单定义ID）
     */
    public String deployFormDefinition (String formSchema, String formDefinitionKey, String formDefinitionName) {
        try {
            ...
            log.info ("[表单引擎] 表单定义部署成功: id={}, key={}, version={}", formSchemaEntity.getId (), formDefinitionKey,
                    newVersion);
    ...
    public String startFormInstance (String formDefinitionKey, String processInstanceId, String taskId,
        ...
        try {
            log.info ("[表单引擎] 开始启动表单实例: formKey={}, processInstanceId={}, taskId={}", formDefinitionKey,
            ...
            log.info ("[表单引擎] 表单实例启动成功: formInstanceId={}, formSchemaId={}", formInstance.getId (),
                    formSchema.getId ());
    ...
    public List<FormInstanceEntity> getFormInstances (String processInstanceId, String taskId, String submitterId) {
        try {
            log.debug ("[表单引擎] 获取表单实例列表: processInstanceId={}, taskId={}, submitterId={}", processInstanceId, taskId,
                    submitterId);

            List<FormInstanceEntity> formInstances = formInstanceDao.findByConditions (processInstanceId, taskId,
                    submitterId != null ? Long.parseLong (submitterId) : null);
    ...
    public Map<String, Object> getFormInfo (String formDefinitionKey) {
        try {
            ...
            log.error ("[表单引擎] 获取表单Schema信息失败: formDefinitionKey={}, error={}", formDefinitionKey, e.getMessage (), e);
    ...
    private boolean isValidFormSchema (JsonNode schemaNode) {
        ...
        return schemaNode != null && schemaNode.has ("type") && "object".equals (schemaNode.get ("type").asText ())
