 ...
 * <p>
 * 专门处理Flowable 7.2.0工作流引擎的异常 严格遵循CLAUDE.md规范： -
...
public class FlowableExceptionHandler {
     ...
     * 记录异常详细指标（用于记录错误码和类别）
     * <p>
     * 注意：不使用 timestamp 作为标签，因为它是高基数标签会导致指标爆炸
     * </p>
     */
    private void recordExceptionDetailMetrics(String errorCode, String category) {
        meterRegistry.counter("workflow.exception.detail", "error_code", errorCode, "category", category).increment();
    }
}
