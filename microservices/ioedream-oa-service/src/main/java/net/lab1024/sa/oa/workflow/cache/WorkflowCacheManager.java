 ...
 * 工作流高级缓存管理器 - 三级缓存架构
 * <p>
 * 三级缓存实现： - L1本地缓存 (Caffeine)：毫秒级响应，TTL 5分钟，容量10000 - L2 Redis缓存：分布式一致性，TTL
 * 30分钟 - L3 应用缓存：进程内持久化，TTL 2小时
 * </p>
 *
 * <p>
 * 严格遵循CLAUDE.md规范： - Manager类是纯Java类，不使用Spring注解 - 通过构造函数注入依赖 -
...
public class WorkflowCacheManager {
    ...
    public static class CacheStatistics {
        ...
        private final Map<CacheLevel, AtomicLong> levelHitCount = new EnumMap<CacheLevel, AtomicLong>(CacheLevel.class);
        private final Map<CacheLevel, AtomicLong> totalResponseTime = new EnumMap<CacheLevel, AtomicLong>(
                CacheLevel.class);
