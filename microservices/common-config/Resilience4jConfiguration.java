...
public class Resilience4jConfiguration {
    ...
    public KeyResolver apiKeyResolver() {
        return exchange -> Mono.just(exchange.getRequest().getPath()
                .value());
    }
    ...
    public RateLimiter databaseRateLimiter(RateLimiterRegistry registry) {
        ...
        rateLimiter.getEventPublisher()
                .onFailure(event ->
                        log.warn("[数据库限流] 限流拒绝, duration={}", event.getElapsedDuration()))
                .onSuccess(event ->
                        log.debug("[数据库限流] 限流通过, duration={}", event.getElapsedDuration()));
    ...
    public Bulkhead databaseBulkhead(BulkheadRegistry registry) {
        ...
        bulkhead.getEventPublisher()
                ...
                .onFinished(event ->
                        log.debug("[数据库舱壁] 调用完成, duration={}", event.getElapsedDuration()));
    ...
    public CircuitBreaker redisCircuitBreaker(CircuitBreakerRegistry registry) {
        CircuitBreakerConfig config = CircuitBreakerConfig.custom()
                ...
                .recordExceptions(
                        redis.clients.jedis.exceptions.JedisConnectionException.class,
    ...
    public CircuitBreaker microserviceCircuitBreaker(CircuitBreakerRegistry registry) {
        CircuitBreakerConfig config = CircuitBreakerConfig.custom()
                ...
                .recordExceptions(
                        org.springframework.cloud.client.circuitbreaker.CircuitBreakerOpenException.class,
                        org.springframework.web.client.ResourceAccessException.class
                )
                .build();
    ...
    public TimeLimiter databaseTimeLimiter(TimeLimiterRegistry registry) {
        ...
        timeLimiter.getEventPublisher()
                .onTimeout(event ->
                        log.warn("[数据库时间限制器] 超时: timeout={}", event.getTimeoutDuration()))
                .onSuccess(event ->
                        log.debug("[数据库时间限制器] 完成: duration={}", event.getElapsedDuration()));
    ...
    public Predicate<Throwable> nonRetryableExceptionPredicate() {
        return throwable -> {
            ...
            return throwable instanceof java.lang.IllegalArgumentException ||
                   throwable instanceof java.security.AccessDeniedException ||
