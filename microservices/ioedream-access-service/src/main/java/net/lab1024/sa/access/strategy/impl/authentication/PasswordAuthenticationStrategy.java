 ...
 * - 设备端通过密码验证识别出人员编号（pin）
 * - 设备端发送请求到软件端：pin=1001, verifytype=0
 * - 软件端接收的是人员编号（pin），不是密码明文
 * </p>
 * <p>
 * <strong>核心职责是验证认证方式是否允许</strong>：
 * - 验证用户是否允许使用密码认证方式
 * - 验证区域配置中是否允许密码认证（某些高安全区域可能禁止密码）
 * - 验证设备配置中是否支持密码认证
 * - 记录密码认证方式（用于统计和审计）
 * </p>
 *
...
public class PasswordAuthenticationStrategy extends AbstractAuthenticationStrategy {
    ...
    protected VerificationResult doAuthenticate(AccessVerificationRequest request) {
        log.debug("[密码认证] 验证认证方式是否允许: userId={}, deviceId={}, areaId={}",
                request.getUserId(), request.getDeviceId(), request.getAreaId());

        // TODO: 后续扩展：检查用户权限配置中是否允许密码认证
        // TODO: 后续扩展：检查区域配置中是否允许密码认证（某些高安全区域可能禁止密码）
        // TODO: 后续扩展：检查设备配置中是否支持密码认证

        // 当前实现：默认允许使用密码认证方式
        // 实际的权限验证在AccessVerificationManager中完成（反潜、互锁、时间段等）

        return VerificationResult.success("密码认证方式验证通过", null, "password");
    }
