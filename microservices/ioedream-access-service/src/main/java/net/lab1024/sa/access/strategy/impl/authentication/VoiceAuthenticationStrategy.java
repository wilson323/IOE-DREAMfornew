 ...
 * - 设备端通过声纹识别（1:N比对）识别出人员编号（pin）
 * - 设备端发送请求到软件端：pin=1001, verifytype=14
 * - 软件端接收的是人员编号（pin），不是声纹特征数据
 * </p>
 * <p>
...
@Slf4j
@Component
public class VoiceAuthenticationStrategy extends AbstractAuthenticationStrategy {

    @Override
    protected VerificationResult doAuthenticate(AccessVerificationRequest request) {
        log.debug("[声纹认证] 验证认证方式是否允许: userId={}, deviceId={}, areaId={}",
                request.getUserId(), request.getDeviceId(), request.getAreaId());

        // TODO: 后续扩展：检查用户权限配置中是否允许声纹认证
        // TODO: 后续扩展：检查区域配置中是否允许声纹认证
        // TODO: 后续扩展：检查设备配置中是否支持声纹认证

        // 当前实现：默认允许使用声纹认证方式
        // 实际的权限验证在AccessVerificationManager中完成（反潜、互锁、时间段等）

        return VerificationResult.success("声纹认证方式验证通过", null, "voice");
    }

    @Override
    public String getStrategyName() {
        return "VoiceAuthenticationStrategy";
    }

    @Override
    public VerifyTypeEnum getVerifyType() {
        return VerifyTypeEnum.VOICE;
    }
}
