...
@Slf4j
@RestController
@PermissionCheck(value = "ACCESS_MANAGE", description = "门禁反潜回管理权限")
@RequestMapping("/api/v1/access/anti-passback")
@Tag(name = "反潜回管理", description = "反潜回检查、配置管理、异常处理、统计分析等API")
@Validated
...
public class AntiPassbackController {
    ...
    @Operation(
        summary = "执行反潜回检查",
        description = "检查用户是否存在于反潜回队列中，支持多种反潜回模式"
    )
    @PostMapping("/check")
    ...
    @Operation(
        summary = "清理用户反潜回记录",
        description = "清理用户的反潜回记录，通常在用户完成退出流程后调用"
    )
    ...
    @Operation(
        summary = "重置用户反潜回状态",
        description = "管理员手动重置用户反潜回状态，用于处理异常情况"
    )
    ...
    public CompletableFuture<ResponseDTO<Void>> resetUserAntiPassbackStatus(
            ...
            @Parameter(description = "重置原因", required = true, example = "报告异常")
    ...
    @Operation(
        summary = "获取反潜回统计信息",
        description = "获取反潜回系统状态标识统计数据和分析报告"
    )
    ...
    public CompletableFuture<ResponseDTO<Map<Long, Object>>> batchCheckAntiPassbackStatus(
            @Parameter(description = "用户ID列表，使用逗号分隔", required = true)
