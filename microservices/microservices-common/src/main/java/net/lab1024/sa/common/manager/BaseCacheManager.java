...
public abstract class BaseCacheManager {
    ...
    protected <T> T getCache(String cacheKey, DataLoader<T> dbLoader) {
        return getCache(cacheKey, dbLoader, REDIS_EXPIRE_MINUTES);
    }
     ...
     * @return 数据对象,可能为null
     */
    protected <T> T getCache(String cacheKey, DataLoader<T> dbLoader, long expireTime) {
        // 1. 先查L1本地缓存
        ...
        if (dbLoader != null) {
            ...
            if (data != null) {
                // 4. 异步写入缓存(使用指定的过期时间)
                setCacheAsync(cacheKey, data, expireTime);
            }
        }
    ...
    protected void setCache(String cacheKey, Object data) {
        setCache(cacheKey, data, REDIS_EXPIRE_MINUTES);
    }
     ...
     * @param expireTime 过期时间(秒)
     */
    protected void setCache(String cacheKey, Object data, long expireTime) {
        try {
            localCache.put(cacheKey, data);
            redisTemplate.opsForValue().set(cacheKey, data, expireTime, TimeUnit.SECONDS);
            log.debug("缓存设置成功, cacheKey: {}, expireTime: {}秒", cacheKey, expireTime);
        } catch (Exception e) {
    ...
    protected void setCacheAsync(String cacheKey, Object data) {
        setCacheAsync(cacheKey, data, REDIS_EXPIRE_MINUTES);
    }
     ...
     */
    @Async
    protected void setCacheAsync(String cacheKey, Object data, long expireTime) {
        try {
            localCache.put(cacheKey, data);
            redisTemplate.opsForValue().set(cacheKey, data, expireTime, TimeUnit.SECONDS);
            log.debug("缓存异步设置成功, cacheKey: {}, expireTime: {}秒", cacheKey, expireTime);
        } catch (Exception e) {
