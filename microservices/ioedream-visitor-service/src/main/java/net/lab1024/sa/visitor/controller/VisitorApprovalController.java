...
public class VisitorApprovalController {
    ...
    @Operation(
        summary = "批量审批预约",
        description = "批量处理多个预约的审批决策，提高审批效率"
    )
    @PostMapping("/batch")
    public CompletableFuture<ResponseDTO<List<Long>>> batchApproveAppointments(
            @Parameter(description = "预约ID列表", required = true)
