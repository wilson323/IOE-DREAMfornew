...
public class MobileAccountInfoManager {
    ...
    public MobileAccountInfoVO getAccountInfo(Long accountId, Long userId) {
        ...
        try {
            ...
            if (account == null) {
                log.warn("[账户信息Manager] 账户不存在: accountId={}, userId={}", accountId, userId);
            ...
            throw new SystemException("GET_ACCOUNT_INFO_ERROR", "获取账户信息失败", e);
        }
    }
    ...
    private AccountConsumeStats getAccountConsumeStats(Long accountId) {
        try {
            ...
            // 注意：Map.class 是原始类型，方法级别的 @SuppressWarnings("rawtypes") 已覆盖
            net.lab1024.sa.common.dto.ResponseDTO response = gatewayServiceClient.callConsumeService(
