...
import net.lab1024.sa.common.cache.RedisUtil;
import net.lab1024.sa.consume.dao.ConsumeRecordDao;
import net.lab1024.sa.consume.domain.analysis.BehaviorAnalysis;
import net.lab1024.sa.consume.domain.entity.ConsumeRecordEntity;
import net.lab1024.sa.consume.domain.pattern.ConsumptionPattern;
import net.lab1024.sa.consume.domain.pattern.DevicePattern;
import net.lab1024.sa.consume.domain.pattern.LocationPattern;
import net.lab1024.sa.consume.domain.pattern.TimePattern;
import net.lab1024.sa.consume.domain.result.*;
import net.lab1024.sa.consume.domain.vo.*;
import net.lab1024.sa.consume.service.AbnormalDetectionService;
...
public class AbnormalDetectionServiceImpl implements AbnormalDetectionService {
    ...
    public LocationAnomalyResult detectLocationAnomaly(@NotNull Long personId, @NotNull String currentLocation,
        ...
        try {
            ...
            String message = isAbnormal ? "检测到新的消费位置，存在异常风险" : "消费位置正常，符合历史模式";
    ...
    public TimeAnomalyResult detectTimeAnomaly(@NotNull Long personId, LocalDateTime operationTime,
        ...
        try {
            ...
            String message = isAbnormalHour ? "检测到异常时间操作，可能存在风险" : "操作时间正常，符合用户历史模式";
    ...
    public AmountAnomalyResult detectAmountAnomaly(@NotNull Long personId, @NotNull BigDecimal amount,
        ...
        try {
            ...
            String message = isAbnormal ? "检测到异常消费金额，可能存在风险" : "消费金额正常，符合用户历史模式";
    ...
    public DeviceAnomalyResult detectDeviceAnomaly(@NotNull Long personId, @NotNull Long deviceId,
        ...
        try {
            ...
            String message = isNewDevice ? "检测到新设备使用，需要关注" : "设备使用正常，符合用户历史模式";
    ...
    public FrequencyAnomalyResult detectFrequencyAnomaly(@NotNull Long personId, @NotNull String operationType,
        ...
        try {
            ...
            String message = isAbnormal ? "检测到异常操作频率，可能存在风险" : "操作频率正常，符合用户历史模式";
    ...
    // 其他接口的默认实现
    @Override
    public SequenceAnomalyResult detectSequenceAnomaly(@NotNull Long personId,
            @NotNull List<OperationEventEntity> operations) {
    ...
    private AbnormalDetectionResult evaluateOverallAnomaly(List<AbnormalDetectionResult> results) {
        ...
        return AbnormalDetectionResult.builder()
                ...
                .message(abnormalCount > 0 ? String.format("检测到%d项异常，建议人工审核", abnormalCount) : "综合检测正常")
                .detectionTime(LocalDateTime.now())
                .build();
    }
