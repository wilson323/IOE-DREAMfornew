...
```bash
# 问题表现
import jakarta.servlet.http.HttpServletRequest;  // ❌ 错误
import jakarta.validation.Valid;              // ❌ 错误
...
// 批量修复
find . -name "*.java" -exec sed -i 's/@Resource/@Resource/g' {} \;
```
...
# 3. 依赖注入检查
autowired_count=$(grep -r "@Resource" --include="*.java" . | wc -l)
echo "💉 依赖注入检查: $autowired_count 个@Resource (目标: 0)"
...
- **❌ 使用@Resource字段注入** - Spring Boot 3.x要出问题的！
...
grep -r "@Resource" --include="*.java" . | wc -l            # 必须=0
```
