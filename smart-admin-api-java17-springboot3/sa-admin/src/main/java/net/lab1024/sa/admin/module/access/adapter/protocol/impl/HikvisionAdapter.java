...
public class HikvisionAdapter implements AccessProtocolInterface {
    ...
    private static final Logger log = LoggerFactory.getLogger(HikvisionAdapter.class);
    ...
    public DeviceDispatchResult dispatchBiometricData(SmartDeviceEntity device, Map<String, Object> biometricData) throws Exception {
        try {
            ...
            } else if ("CARD".equalsIgnoreCase(biometricType)) {
                success = dispatchCardData(device, biometricData);
                message = success ? "卡片数据下发成功" : "卡片数据下发失败";
            } else {
    ...
    public DeviceDispatchResult remoteOpenDoor(SmartDeviceEntity device, String doorId) throws Exception {
        try {
            ...
            if ("HTTP".equalsIgnoreCase(protocolType) || "ISAPI".equalsIgnoreCase(protocolType)) {
                success = remoteOpenDoorViaIsapi(device, doorId);
     ...
     * 测试ISAPI连接
     */
    private boolean testIsapiConnection(SmartDeviceEntity device) throws Exception {
        // TODO: 实现海康威视ISAPI连接测试
        // 1. 发送HTTP GET请求到ISAPI接口
        // 2. 验证响应状态和设备信息
        log.debug("测试海康威视ISAPI连接: http://{}:{}/ISAPI/System/deviceInfo",
    ...
    private Map<String, Object> convertToHikvisionPersonData(Map<String, Object> personData) {
        ...
        hkData.put("doorRight", personData.get("accessLevel")); // 门禁权限
        hkData.put("beginTime", personData.get("validFrom"));
        hkData.put("endTime", personData.get("validTo"));

        // 海康威视特有的字段
        hkData.put("personType", "normal"); // 人员类型
        hkData.put("gender", "unknown"); // 性别
        hkData.put("nation", "unknown"); // 民族
    ...
    private Map<String, Object> convertToHikvisionConfigData(Map<String, Object> configData) {
        ...
        // 海康威视配置数据格式转换
        hkData.put("door1", configData.get("openMethod"));
        hkData.put("door2", configData.get("openMethod"));
        hkData.put("lockType", configData.get("lockType"));
        hkData.put("unlockTime", configData.get("unlockDuration"));
        hkData.put("openDoorTime", configData.get("openDelay"));
        hkData.put("antiPassback", configData.get("antiPassbackEnabled"));
    ...
    // 占位方法 - 需要根据实际海康威视协议实现
    private String getDeviceModel(SmartDeviceEntity device) { return "Hikvision-DS-K2801"; }
    private String getDeviceSerialNumber(SmartDeviceEntity device) { return "Unknown"; }
    private String getDeviceFirmwareVersion(SmartDeviceEntity device) { return "V5.5.0"; }
    private String getDeviceType(SmartDeviceEntity device) { return "Access Controller"; }

    private boolean dispatchPersonDataViaIsapi(SmartDeviceEntity device, Map<String, Object> personData) throws Exception { return true; }
    ...
    private boolean dispatchCardData(SmartDeviceEntity device, Map<String, Object> biometricData) throws Exception { return true; }

    private boolean dispatchConfigViaIsapi(SmartDeviceEntity device, Map<String, Object> configData) throws Exception { return true; }
    private boolean dispatchConfigViaSdk(SmartDeviceEntity device, Map<String, Object> configData) throws Exception { return true; }

    private boolean remoteOpenDoorViaIsapi(SmartDeviceEntity device, String doorId) throws Exception { return true; }
    private boolean remoteOpenDoorViaSdk(SmartDeviceEntity device, String doorId) throws Exception { return true; }

    private Map<String, Object> getStatusViaIsapi(SmartDeviceEntity device) throws Exception {
        Map<String, Object> status = new HashMap<>();
    ...
    private List<Map<String, Object>> queryPersonsViaIsapi(SmartDeviceEntity device) throws Exception { return new ArrayList<>(); }
    private List<Map<String, Object>> queryPersonsViaSdk(SmartDeviceEntity device) throws Exception { return new ArrayList<>(); }

    private boolean deletePersonViaIsapi(SmartDeviceEntity device, Long personId) throws Exception { return true; }
    private boolean deletePersonViaSdk(SmartDeviceEntity device, Long personId) throws Exception { return true; }

    private List<Map<String, Object>> getAccessRecordsViaIsapi(SmartDeviceEntity device, String startTime, String endTime, Integer recordCount) throws Exception { return new ArrayList<>(); }
