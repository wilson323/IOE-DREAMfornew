...
import net.lab1024.sa.admin.module.smart.video.dao.DeviceStatusLogDao;
...
import net.lab1024.sa.base.common.domain.entity.DeviceStatusLogEntity;
...
public class VideoDeviceManager implements VideoDeviceManagerInterface {
     ...
     *
     * @param deviceId 设备ID
     * @param faceFeatures 人脸特征数据
     * @param timestamp 识别时间戳
     * @throws Exception 业务异常
     */
    public void processFaceRecognitionResult(Long deviceId, List<Map<String, Object>> faceFeatures,
            LocalDateTime timestamp) throws Exception {
        log.debug("开始处理人脸识别结果: deviceId={}, featureCount={}", deviceId,
                faceFeatures != null ? faceFeatures.size() : 0);
        ...
        VideoDeviceEntity device = videoDeviceDao.selectById(deviceId);
        if (device == null) {
            log.warn("设备不存在，跳过人脸识别结果处理: deviceId={}", deviceId);
            return;
        }

        // 2. 异步处理每张人脸
        for (Map<String, Object> faceData : faceFeatures) {
            CompletableFuture.runAsync(() -> {
                try {
                    processSingleFaceRecognition(deviceId, faceData, timestamp);
                } catch (Exception e) {
                    log.error("处理单张人脸识别失败: deviceId={}, error={}", deviceId, e.getMessage(),
                            e);
                }
            });
        }

        log.debug("人脸识别结果处理任务已提交: deviceId={}", deviceId);
    }
    ...
    private String sendPTZCommand(VideoDeviceEntity device, String command) throws Exception {
        // 模拟发送 PTZ 指令到设备
        // log.debug("发送PTZ指令到设备 deviceId={}, command={}", device.getDeviceId(), command);
        // 模拟网络延迟
    ...
    private Map<String, Object> getDeviceStatistics(Long deviceId) {
        ...
        // 获取人脸识别统计
        Long todayFaceCount = videoFaceFeatureDao.getTodayFaceCount(deviceId);
        ...
        statistics.put("todayFaceCount", todayFaceCount != null ? todayFaceCount : 0);
    ...
    private void processSingleFaceRecognition(Long deviceId, Map<String, Object> faceData,
            LocalDateTime timestamp) throws Exception {
        // 创建人脸特征记录
        FaceFeatureEntity featureEntity = new FaceFeatureEntity();
        featureEntity.setDeviceId(deviceId);
        featureEntity.setFeatureData(convertToJson(faceData));
        featureEntity.setDetectTime(timestamp);
        featureEntity.setProcessStatus("PROCESSED");
        featureEntity.setCreateTime(LocalDateTime.now());
        featureEntity.setUpdateTime(LocalDateTime.now());

        videoFaceFeatureDao.insert(featureEntity);

        log.debug("单张人脸识别处理完成: deviceId={}, featureId={}", deviceId,
                featureEntity.getFeatureId());
    }
