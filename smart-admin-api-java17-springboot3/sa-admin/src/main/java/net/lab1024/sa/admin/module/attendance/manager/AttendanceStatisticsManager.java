...
public class AttendanceStatisticsManager {
    ...
    public EmployeeMonthlyStatistics generateEmployeeMonthlyStatistics(Long employeeId, String yearMonth) {
        ...
        try {
            // 1. 检查缓存
            String cacheKey = String.format("attendance:statistics:employee:%s:%s", employeeId, yearMonth);
            EmployeeMonthlyStatistics cachedResult = cacheManager.getCacheWithFallback(cacheKey,
            ...
            // 6. 缓存结果
            cacheManager.setCache(cacheKey, statistics);
    ...
    public DepartmentMonthlyStatistics generateDepartmentMonthlyStatistics(Long departmentId, String yearMonth,
        ...
        try {
            // 1. 检查缓存
            String cacheKey = String.format("attendance:statistics:department:%s:%s", departmentId, yearMonth);
            DepartmentMonthlyStatistics cachedResult = cacheManager.getCacheWithFallback(cacheKey,
            ...
            // 5. 缓存结果
            cacheManager.setCache(cacheKey, departmentStatistics);
    ...
    private void calculateEmployeeStatistics(Long employeeId, LocalDate startDate, LocalDate endDate,
        ...
        while (!currentDate.isAfter(endDate)) {
            if (attendanceScheduleRepository.selectEmployeeSchedule(employeeId, currentDate) != null) {
                workDays++;
            }
        ...
        for (AttendanceRecordEntity record : records) {
            ...
            // 累计工作时长
            if (record.getActualWorkHours() != null) {
                totalWorkHours = totalWorkHours.add(record.getActualWorkHours());
            }
        ...
        for (EmployeeMonthlyStatistics empStats : employeeStatistics) {
            ...
            if (empStats.getExceptionTypeCount() != null) {
                empStats.getExceptionTypeCount()
                        .forEach((exceptionType, count) -> summaryStatistics.merge(exceptionType, count, Integer::sum));
            }
        }
    ...
    public Map<LocalDate, Boolean> generateWorkdayStatistics(Long employeeId, LocalDate startDate, LocalDate endDate) {
        ...
        try {
            ...
            while (!currentDate.isAfter(endDate)) {
                AttendanceScheduleEntity schedule = attendanceScheduleRepository.selectEmployeeSchedule(employeeId, currentDate);
    ...
    public void clearStatisticsCache(Long employeeId, String yearMonth) {
        try {
            if (employeeId != null && yearMonth != null) {
                // 清除指定员工指定月份的缓存
                String employeeCacheKey = String.format("attendance:statistics:employee:%s:%s", employeeId, yearMonth);
                cacheManager.removeCacheWithDoubleDelete(employeeCacheKey);
