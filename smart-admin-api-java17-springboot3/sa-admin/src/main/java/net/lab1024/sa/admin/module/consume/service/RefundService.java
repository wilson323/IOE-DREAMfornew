...
import net.lab1024.sa.admin.module.consume.manager.RefundManager;
...
public class RefundService {
    ...
    public ResponseDTO<RefundResultDTO> createRefund(@Valid RefundRequestDTO refundRequest) {
        try {
            ...
            if (validationResult != null) {
                log.warn("退款请求验证失败: {}", validationResult);
                return ResponseDTO.error(UserErrorCode.PARAM_ERROR, validationResult);
            }

            // 2. 查询原消费记录
            ConsumeRecordEntity consumeRecord = consumeRecordDao.selectById(refundRequest.getConsumeRecordId());
            if (consumeRecord == null) {
                return ResponseDTO.error(UserErrorCode.PARAM_ERROR, "原消费记录不存在");
            }

            // 3. 创建退款记录
            RefundRecordEntity refundRecord = createRefundRecord(refundRequest, consumeRecord);
            refundRecord = refundRecordDao.save(refundRecord);
            ...
            return ResponseDTO.error(SystemErrorCode.SYSTEM_ERROR, "创建退款申请失败: " + e.getMessage());
        }
    }
    ...
    public ResponseDTO<String> auditRefund(Long refundId, boolean approved, String auditRemark) {
        try {
            ...
            if (refundRecord == null) {
                return ResponseDTO.error(UserErrorCode.PARAM_ERROR, "退款记录不存在");
            }
            ...
            if (currentStatus == null || currentStatus != RefundStatusEnum.PENDING) {
                return ResponseDTO.error(UserErrorCode.PARAM_ERROR, "退款状态不允许审核，只有待处理状态的退款可以审核");
            }
            ...
            return ResponseDTO.okMsg("审核完成");

        } catch (Exception e) {
            log.error("审核退款申请失败: 退款ID={}", refundId, e);
            return ResponseDTO.error(SystemErrorCode.SYSTEM_ERROR, "审核退款申请失败");
        }
    }
    ...
    public ResponseDTO<Page<RefundRecordEntity>> queryRefundRecords(RefundQueryDTO queryDTO) {
        try {
            ...
            return ResponseDTO.error(SystemErrorCode.SYSTEM_ERROR, "查询退款记录失败");
        }
    }
    ...
    public ResponseDTO<Map<String, Object>> getRefundStatistics(Long userId, LocalDateTime startDate,
        ...
        try {
            ...
            return ResponseDTO.error(SystemErrorCode.SYSTEM_ERROR, "获取退款统计失败");
        }
    }
        ...
        try {
            ...
            for (Long refundId : refundIds) {
                try {
                    ...
                    if (ResponseDTO.OK_CODE.equals(auditResult.getCode())) {
                        successCount++;
                    } else {
                        failureCount++;
                    }
            ...
            return ResponseDTO.error(SystemErrorCode.SYSTEM_ERROR, "批量审核退款失败");
        }
    }
    ...
    public ResponseDTO<RefundRecordEntity> getRefundDetail(Long refundId) {
        try {
            ...
            return ResponseDTO.error(SystemErrorCode.SYSTEM_ERROR, "获取退款详情失败");
        }
    }
    ...
    public ResponseDTO<String> cancelRefund(String refundNo) {
        try {
            ...
            return ResponseDTO.okMsg("取消退款申请成功");

        } catch (Exception e) {
            log.error("取消退款申请失败: refundNo={}", refundNo, e);
            return ResponseDTO.error(SystemErrorCode.SYSTEM_ERROR, "取消退款申请失败: " + e.getMessage());
        }
    }
    ...
    public ResponseDTO<RefundResultDTO> resubmitRefund(String refundNo, RefundRequestDTO refundRequest) {
        try {
            ...
            if (originalRefund == null) {
                return ResponseDTO.error(UserErrorCode.PARAM_ERROR, "原退款记录不存在");
            }
            ...
            newRefundRecord = refundRecordDao.save(newRefundRecord);
            ...
            return ResponseDTO.error(SystemErrorCode.SYSTEM_ERROR, "重新提交退款申请失败: " + e.getMessage());
        }
    }
    ...
    public ResponseDTO<Map<String, Object>> getRefundStatusStatistics() {
        try {
            ...
            return ResponseDTO.error(SystemErrorCode.SYSTEM_ERROR, "获取退款状态统计失败: " + e.getMessage());
        }
    }
}
