...
import net.lab1024.sa.base.config.SystemEnvironmentConfig;
...
import net.lab1024.sa.admin.common.swagger.SmartOperationCustomizer;
import net.lab1024.sa.base.constant.SwaggerTagConst;

/**
 * springdoc-openapi 配置 nginx配置前缀时如果需要访问【/swagger-ui/index.html】需添加额外nginx配置 location /v3/api-docs/
...
public class SwaggerConfig {
    ...
    public static final String[] SWAGGER_WHITELIST =
            {"/swagger-ui/**", "/swagger-ui/index.html", "/swagger-ui.html", "/swagger-ui.html/**",
                    "/v3/api-docs", "/v3/api-docs/**", "/doc.html",};

    @Bean
    @Primary
    public OpenAPI api() {
        return new OpenAPI().components(components()).info(new Info().title("SmartAdmin 3.X 接口文档")
                .contact(new Contact().name("1024创新实验室").email("lab1024@163.com")
                        .url("https://1024lab.net"))
                .version("v3.X").description(
                        "<font color=\"#DC143C\">**以「高质量代码」为核心，「简洁、高效、安全」**</font>基于 SpringBoot + Sa-Token + Mybatis-Plus 和 Vue3 + Vite5 + Ant Design (同时支持JavaScript和TypeScript双版本) 的快速开发平台。"
    ...
    private Components components() {
        return new Components().addSecuritySchemes(RequestHeaderConst.TOKEN,
                new SecurityScheme().scheme("Bearer").description("请输入token,格式为[Bearer xxxxxxxx]")
    ...
    public GroupedOpenApi businessApi() {
        return GroupedOpenApi.builder().group("业务接口").pathsToMatch("/**")
                .pathsToExclude(SwaggerTagConst.Support.URL_PREFIX + "/**")
                .addOperationCustomizer(new SmartOperationCustomizer()).build();

    }

    @Bean
    public GroupedOpenApi supportApi() {
        return GroupedOpenApi.builder().group("支撑接口(Support)")
                .pathsToMatch(SwaggerTagConst.Support.URL_PREFIX + "/**")
                .addOperationCustomizer(new SmartOperationCustomizer()).build();
    }

    /**
     * 以下代码可以用于设置 /swagger-ui/index.html 的serverBaseUrl 如果使用knife4j则不需要
     *
     * @param openAPI
    ...
    public OpenAPIService openApiBuilder(Optional<OpenAPI> openAPI, SecurityService securityParser,
        ...
        return new OpenAPIService(openAPI, securityParser, springDocConfigProperties,
                propertyResolverUtils, openApiBuilderCustomizers, Optional.of(list),
                javadocProvider);
    }
}
