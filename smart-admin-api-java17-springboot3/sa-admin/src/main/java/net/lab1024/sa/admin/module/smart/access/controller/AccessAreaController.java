...
public class AccessAreaController {
    ...
    public ResponseDTO<List<AccessAreaTreeVO>> getAreaTree(
            @Parameter(description = "父级区域ID，0表示根区域", example = "0")
            @RequestParam(defaultValue = "0") Long parentId,

            @Parameter(description = "是否包含子区域", example = "true")
    ...
    @SaCheckPermission("smart:access:area:query")
    public ResponseDTO<AccessAreaEntity> getAreaDetail(
            @Parameter(description = "区域ID", required = true, example = "1")
            @PathVariable @NotNull(message = "区域ID不能为空") Long areaId) {
    ...
    public ResponseDTO<PageResult<AccessAreaEntity>> getAreaPage(
            PageParam pageParam,
            @Parameter(description = "区域名称", example = "主园区")
            @RequestParam(required = false) String areaName,

            @Parameter(description = "区域类型", example = "1")
            @RequestParam(required = false) Integer areaType,

            @Parameter(description = "区域状态", example = "1")
            @RequestParam(required = false) Integer status) {

        log.info("分页查询区域，areaName: {}, areaType: {}, status: {}", areaName, areaType, status);
        PageResult<AccessAreaEntity> pageResult = accessAreaService.getAreaPage(pageParam, areaName, areaType, status);
    ...
    @SaCheckPermission("smart:access:area:add")
    ...
    @SaCheckPermission("smart:access:area:update")
    ...
    @SaCheckPermission("smart:access:area:delete")
    ...
    public ResponseDTO<List<Object>> getAreaDevices(
            ...
            @Parameter(description = "是否包含子区域设备", example = "false")
    ...
    public ResponseDTO<String> moveArea(
            ...
            @Parameter(description = "新父级区域ID", required = true, example = "2")
            @RequestParam @NotNull(message = "新父级区域ID不能为空") Long newParentId) {
    ...
    public ResponseDTO<String> updateAreaStatus(
            ...
            @Parameter(description = "区域状态", required = true, example = "1")
            @RequestParam @NotNull(message = "区域状态不能为空") Integer status) {
    ...
    public ResponseDTO<Boolean> validateAreaCode(
            @Parameter(description = "区域编码", required = true, example = "AREA_001")
            @RequestParam @NotNull(message = "区域编码不能为空") String areaCode,

            @Parameter(description = "排除的区域ID（编辑时使用）", example = "1")
