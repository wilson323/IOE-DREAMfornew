...
public class AttendanceRuleEngine {
    ...
    private final Map<String, List<AttendanceScheduleEntity>> scheduleCache = new ConcurrentHashMap<>();
    ...
    private AttendanceRuleEntity getEmployeeRule(Long employeeId) {
        return ruleCache.computeIfAbsent(employeeId, k -> {
            try {
     ...
     * 清除员工规则缓存
     */
    public void clearEmployeeCache(Long employeeId) {
        ruleCache.remove(employeeId);
        scheduleCache.remove("employee_" + employeeId);
