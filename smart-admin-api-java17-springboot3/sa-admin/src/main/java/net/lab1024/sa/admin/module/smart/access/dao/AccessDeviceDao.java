...
public interface AccessDeviceDao extends BaseMapper<AccessDeviceEntity> {
     ...
     * 查询指定区域的设备列表
     */
    @Select("SELECT * FROM t_smart_access_device WHERE area_id = #{areaId} AND deleted_flag = 0 ORDER BY create_time ASC")
    List<AccessDeviceEntity> selectByAreaId(@Param("areaId") Long areaId);

    /**
     * 查询在线设备列表
     */
    @Select("SELECT * FROM t_smart_access_device WHERE online_status = 1 AND enabled = 1 AND deleted_flag = 0 ORDER BY last_heartbeat_time DESC")
    List<AccessDeviceEntity> selectOnlineDevices();

    /**
     * 查询离线设备列表
     */
    @Select("SELECT * FROM t_smart_access_device WHERE online_status = 0 AND enabled = 1 AND deleted_flag = 0 ORDER BY last_comm_time ASC")
    List<AccessDeviceEntity> selectOfflineDevices();

    /**
     * 查询需要维护的设备列表
     */
    @Select("SELECT * FROM t_smart_access_device WHERE next_maintenance_time < NOW() AND enabled = 1 AND deleted_flag = 0 ORDER BY next_maintenance_time ASC")
    List<AccessDeviceEntity> selectDevicesNeedingMaintenance();

    /**
     * 查询心跳超时的设备列表
     */
    @Select("SELECT * FROM t_smart_access_device WHERE " +
     ...
     */
    @Select("<script>" +
            "UPDATE t_smart_access_device SET online_status = #{status}, update_time = NOW() " +
     ...
     * 统计设备数量
     */
    @Select("SELECT COUNT(*) FROM t_smart_access_device WHERE deleted_flag = 0")
    Long countTotalDevices();

    /**
     * 统计在线设备数量
     */
    @Select("SELECT COUNT(*) FROM t_smart_access_device WHERE online_status = 1 AND deleted_flag = 0")
    Long countOnlineDevices();

    /**
     * 统计指定区域的设备数量
     */
    @Select("SELECT COUNT(*) FROM t_smart_access_device WHERE area_id = #{areaId} AND deleted_flag = 0")
    ...
    @Select("SELECT access_device_type, COUNT(*) as count " +
            "FROM t_smart_access_device WHERE deleted_flag = 0 " +
     ...
     */
    @Select("<script>" +
            "SELECT * FROM t_smart_access_device WHERE deleted_flag = 0 " +
            "AND area_id IN " +
