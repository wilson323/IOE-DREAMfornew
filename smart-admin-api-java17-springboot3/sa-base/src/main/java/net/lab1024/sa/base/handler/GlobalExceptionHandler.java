...
public class GlobalExceptionHandler {
    ...
    public ResponseDTO<String> notLoginException(NotLoginException e) {
        log.warn("Sa-Token未登录异常,URL:{}, message:{}", getCurrentRequestUrl(), e.getMessage());
        if (SystemEnvironmentEnum.PROD != systemEnvironment.getCurrentEnvironment()) {
            return ResponseDTO.error(UserErrorCode.NO_PERMISSION, "请先登录: " + e.getMessage());
        }
        return ResponseDTO.error(UserErrorCode.NO_PERMISSION, "请先登录");
    }
    ...
    public ResponseDTO<String> notRoleException(NotRoleException e) {
        log.warn("Sa-Token角色异常,URL:{}, message:{}", getCurrentRequestUrl(), e.getMessage());
        if (SystemEnvironmentEnum.PROD != systemEnvironment.getCurrentEnvironment()) {
            return ResponseDTO.error(UserErrorCode.NO_PERMISSION, "角色权限不足: " + e.getMessage());
        }
        return ResponseDTO.error(UserErrorCode.NO_PERMISSION, "角色权限不足");
    }
    ...
    public ResponseDTO<String> saTokenException(SaTokenException e) {
        log.warn("Sa-Token异常,URL:{}, message:{}", getCurrentRequestUrl(), e.getMessage());
        if (SystemEnvironmentEnum.PROD != systemEnvironment.getCurrentEnvironment()) {
            return ResponseDTO.error(UserErrorCode.NO_PERMISSION, "认证异常: " + e.getMessage());
        }
        return ResponseDTO.error(UserErrorCode.NO_PERMISSION, "认证异常");
    }
    ...
    public ResponseDTO<String> permissionException(NotPermissionException e) {
        log.warn("Sa-Token权限异常,URL:{}, message:{}", getCurrentRequestUrl(), e.getMessage());
        // 开发环境 更方便调试
        if (SystemEnvironmentEnum.PROD != systemEnvironment.getCurrentEnvironment()) {
            return ResponseDTO.error(UserErrorCode.NO_PERMISSION, "权限不足: " + e.getMessage());
        }
        return ResponseDTO.error(UserErrorCode.NO_PERMISSION, "权限不足");
    }
