...
import net.lab1024.sa.visitor.domain.form.VisitorReservationForm;
import net.lab1024.sa.visitor.domain.vo.VisitorReservationVO;
import net.lab1024.sa.visitor.domain.vo.VisitorRecordVO;
...
public class VisitorController {
    ...
    public ResponseDTO<String> createReservation(@Valid @RequestBody VisitorReservationForm form) {
        ...
        // 转换为实体对象
        VisitorReservationEntity reservation = new VisitorReservationEntity();
        // TODO: 使用SmartBeanUtil.copyProperties进行对象转换
        reservation.setVisitorName(form.getVisitorName());
        reservation.setVisitorPhone(form.getVisitorPhone());
        reservation.setVisitorEmail(form.getVisitorEmail());
        reservation.setVisitorCompany(form.getVisitorCompany());
        reservation.setVisitorIdCard(form.getVisitorIdCard());
        reservation.setVisitorPhoto(form.getVisitorPhoto());
        reservation.setVisitPurpose(form.getVisitPurpose());
        reservation.setVisitDate(form.getVisitDate());
        reservation.setVisitStartTime(form.getVisitStartTime());
        reservation.setVisitEndTime(form.getVisitEndTime());
        reservation.setVisitAreaIds(form.getVisitAreaIds());
        reservation.setHostUserId(form.getHostUserId());
        reservation.setHostUserName(form.getHostUserName());
        reservation.setHostDepartment(form.getHostDepartment());
        reservation.setRemarks(form.getRemarks());
    ...
    public ResponseDTO<String> updateReservation(@Valid @RequestBody VisitorReservationForm form) {
        ...
        VisitorReservationEntity reservation = new VisitorReservationEntity();
        reservation.setReservationId(form.getReservationId());
        reservation.setVisitorName(form.getVisitorName());
        reservation.setVisitorPhone(form.getVisitorPhone());
        reservation.setVisitorEmail(form.getVisitorEmail());
        reservation.setVisitorCompany(form.getVisitorCompany());
    ...
    public ResponseDTO<PageResult<VisitorReservationVO>> getReservationPage(
        ...
        // TODO: 实现实体到VO的转换
        // voPage.setRows(convertToVOList(entityPage.getRows()));
    ...
    public ResponseDTO<VisitorReservationVO> getReservationDetail(
        ...
        // TODO: 转换为VO对象
        // VisitorReservationVO vo = convertToVO(result.getData());
    ...
    public ResponseDTO<String> generateQrCode(
            @Parameter(description = "预约ID") @NotNull @RequestParam Long reservationId) {

        log.info("生成访客二维码: {}", reservationId);
        return visitorService.generateQrCode(reservationId);
    }
