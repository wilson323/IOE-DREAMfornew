...
**重构目标**：构建统一的权限验证系统，整合区域、餐别、定值、限额等多维度权限验证逻辑，支持不同经营模式的权限校验。
...
| 经营模式 | 主要验证点 | 次要验证点 | 不适用验证 |
|---------|-----------|-----------|-----------|
| **餐别制<br/>(manage_mode=1)** | ✅ 餐别权限<br/>✅ 就餐时间<br/>✅ 定值配置 | ✅ 区域权限<br/>✅ 余额限额 | ❌ 商品权限<br/>❌ 库存检查 |
| **超市制<br/>(manage_mode=2)** | ✅ 商品权限<br/>✅ 库存检查<br/>✅ 商品价格 | ✅ 区域权限<br/>✅ 余额限额 | ❌ 餐别权限<br/>❌ 就餐时间<br/>❌ 定值配置 |
| **混合模式<br/>(manage_mode=3)** | ✅ 区域权限<br/>✅ 余额限额 | ✅ 根据消费方式<br/>动态验证 | - |
...
flowchart TD
    ...
    B -->|manage_mode=1<br/>餐别制| C1[餐别制验证链]
    B -->|manage_mode=2<br/>超市制| C2[超市制验证链]
    B -->|manage_mode=3<br/>混合模式| C3[混合模式验证链]
...
sequenceDiagram
    ...
    C->>V: 批量验证请求[100个账户]
    V->>V: 分组去重
    Note over V: 按账户类别分组<br/>减少查询次数
    
    V->>Cache: 批量获取账户类别配置
    Cache-->>V: 返回80个(20个Miss)
    
    V->>DB: 批量查询缺失配置
    Note over DB: 使用IN查询<br/>一次性获取20个
    DB-->>V: 返回20个配置
    
    V->>Cache: 批量写入缓存(Pipeline)
    
    V->>V: 并行验证(Stream并行流)
    Note over V: 使用线程池<br/>100个账户并行验证
    ...
    V->>V: 聚合结果
    V-->>C: 返回100个验证结果
    Note over C: 总耗时 < 200ms<br/>(vs 原3000ms)
```
...
flowchart TD
    ...
    N -->|是<br/>静态权限| O[缓存结果30分钟]
    N -->|否<br/>动态数据| P[不缓存<br/>如余额/次数]
...
CREATE TABLE POSID_PERMISSION_LOG (
    ...
    -- 验证详情
    validation_chain TEXT COMMENT '验证链执行详情(JSON)',
    /*
    示例：
    [
        {"validator": "AccountStatusValidator", "result": true, "duration": 2},
        {"validator": "AreaPermissionValidator", "result": true, "duration": 5},
        {"validator": "MealPermissionValidator", "result": false, "duration": 3, "reason": "无权使用该餐别"}
    ]
    */
    ...
    INDEX idx_result(validation_result, validation_time) COMMENT '结果索引',
    INDEX idx_validator(failure_validator, validation_time) COMMENT '失败验证器索引',
    INDEX idx_time(validation_time) COMMENT '时间索引（用于清理历史数据）'
...
```mermaid
erDiagram
    POSID_ACCOUNT ||--|| POSID_ACCOUNTKIND : "所属类别"
    POSID_ACCOUNTKIND }o--o{ POSID_AREA : "通过area_config JSON关联"
    POSID_AREA ||--o{ POSID_MEAL_CATEGORY : "通过meal_categories JSON关联"
    POSID_MEAL ||--|| POSID_MEAL_CATEGORY : "所属分类"
    
    POSID_ACCOUNT ||--o{ POSID_PERMISSION_LOG : "验证日志"
    
    POSID_ACCOUNT {
        VARCHAR id PK
        VARCHAR account_kind_id FK
        INT balance "余额"
        BOOLEAN available "状态"
    }
    
    POSID_ACCOUNTKIND {
        VARCHAR id PK
        JSON area_config "区域权限配置"
        JSON mode_config "消费模式配置"
        BOOLEAN is_attendance_consume "考勤消费"
    }
    
    POSID_AREA {
        VARCHAR id PK
        TEXT meal_categories "可用餐别分类JSON"
        VARCHAR parent_id FK "父区域"
    }
    ...
    POSID_PERMISSION_LOG {
        ...
        VARCHAR area_id FK
        VARCHAR meal_id FK
        BOOLEAN validation_result "验证结果"
        VARCHAR failure_reason "失败原因"
        TEXT validation_chain "验证链详情"
        INT total_duration "总耗时"
        DATETIME validation_time "验证时间"
    }
```

**关键说明：**
- 账户类别通过 `area_config` JSON字段关联区域（无需关联表）
- 区域通过 `meal_categories` JSON字段配置支持的餐别分类
- 权限验证从JSON配置中读取关联关系
- 提供Redis反向索引、统计表、SQL视图等补偿机制（详见04文档）
...
| 今日消费次数 | `account:today:times:{accountId}` | String | 到当天23:59 | 用于限额验证 |
| 今日消费金额 | `account:today:money:{accountId}` | String | 到当天23:59 | 用于限额验证 |
| 本月消费次数 | `account:month:times:{accountId}:{yyyyMM}` | String | 到月底 | 用于限额验证 |
| 本月消费金额 | `account:month:money:{accountId}:{yyyyMM}` | String | 到月底 | 用于限额验证 |
| 验证器缓存 | `validator:config:{validatorName}` | String (JSON) | 1小时 | 验证器配置 |
...
public class AreaPermissionValidator extends AbstractPermissionValidator {
    ...
    protected ValidationResult doValidate(ValidationContext context) {
        ...
        if (!effectiveAreas.contains(areaId)) {
            return ValidationResult.fail("无权在该区域消费");
        }
        
        // 检查区域是否启用
        PosIDArea area = context.getArea();
        if (area == null || !area.getAvailable()) {
            return ValidationResult.fail("该区域已禁用");
        }
...
public class BalanceValidator extends AbstractPermissionValidator {
    ...
    protected ValidationResult doValidate(ValidationContext context) {
        ...
        if (totalBalance < consumeMoney) {
            return ValidationResult.fail(
                String.format("余额不足（需要%.2f元，当前%.2f元）", 
    ...
    protected ValidationResult doValidate(ValidationContext context) {
        ...
        if (kind.getDateMaxCount() != null && kind.getDateMaxCount() > 0) {
            ...
            if (todayTimes >= kind.getDateMaxCount()) {
                return ValidationResult.fail(
                    String.format("已达日消费次数限额（%d次）", kind.getDateMaxCount())
                );
            }
        }
        ...
        if (kind.getDateMaxMoney() != null && kind.getDateMaxMoney() > 0 && consumeMoney != null) {
            ...
            if (todayMoney + consumeMoney > kind.getDateMaxMoney()) {
                return ValidationResult.fail(
                    String.format("将超出日消费金额限额（限额%.2f元，已消费%.2f元）",
        ...
        if (kind.getMonthMaxCount() != null && kind.getMonthMaxCount() > 0) {
            ...
            if (monthTimes >= kind.getMonthMaxCount()) {
                return ValidationResult.fail(
                    String.format("已达月消费次数限额（%d次）", kind.getMonthMaxCount())
                );
            }
        }
        ...
        if (kind.getPerMaxMoney() != null && kind.getPerMaxMoney() > 0 && consumeMoney != null) {
            if (consumeMoney > kind.getPerMaxMoney()) {
                return ValidationResult.fail(
                    String.format("超出单次消费金额限额（限额%.2f元）",
...
| 需求 | 原设计 | 重构后 |
|------|-------|--------|
| 新增验证规则 | 修改多个Service | 新增Validator类 |
| 调整验证顺序 | 修改代码 | 修改@Order注解 |
| 禁用某个验证 | 注释代码 | 配置isEnabled() |
| 验证结果缓存 | 不支持 | 自动支持 |
...
1. **责任链模式**：9个验证器按优先级执行，失败即中断
2. **多级缓存**：L1本地缓存+L2Redis+L3数据库
3. **静态权限缓存**：分离静态和动态权限，提升缓存命中率
4. **批量优化**：并行流+Pipeline，批量性能提升10倍
5. **完整日志**：验证链详情、耗时、缓存命中率全记录

### 验证器列表

1. **AccountStatusValidator** - 账户状态验证
2. **AreaPermissionValidator** - 区域权限验证
3. **MealPermissionValidator** - 餐别权限验证
4. **TimeWindowValidator** - 时间窗口验证
5. **BalanceValidator** - 余额检查
6. **LimitValidator** - 限额检查（日/月/次）
...
**瓶颈点识别：**
1. 🔴 **数据库连接池耗尽**：默认10个连接，高并发时等待
2. 🔴 **Redis连接超时**：单连接串行，无连接池
3. 🔴 **缓存穿透**：不存在的账户重复查询数据库
4. 🟡 **验证器串行执行**：无法利用多核CPU
5. 🟡 **大量日志写入**：同步写入阻塞主流程
...
public class BloomFilterProtection {
    ...
    public void initBloomFilter() {
        ...
        log.info("布隆过滤器初始化完成，加载{}个账户", accountIds.size());
    }
...
public class OptimizedPermissionService {
    ...
    public PermissionValidationResult validate(PermissionValidationRequest request) {
        ...
        if (!bloomFilter.mightContain(request.getAccountId())) {
            return PermissionValidationResult.fail("账户不存在");
        }
...
public class ParallelValidationService {
    ...
    public PermissionValidationResult parallelValidate(ValidationContext context) {
        ...
        List<PermissionValidator> sequentialValidators = validators.stream()
            .filter(v -> v.getOrder() <= 4)  // 前4个有依赖关系
            .sorted(Comparator.comparing(PermissionValidator::getOrder))
            .collect(Collectors.toList());
...
public class CacheBreakdownProtection {
    ...
    public Object getWithMutex(String key, Supplier<Object> dataLoader) {
        ...
        try {
            ...
            throw new RuntimeException("获取缓存锁中断", e);
        }
    }
}
```
...
public class RedisFallbackHandler {
    ...
    public Object getWithFallback(String key) {
        try {
            ...
            log.warn("Redis访问失败，使用本地缓存降级: {}", e.getMessage());
...
public class ValidationDegradationStrategy {
    ...
    public PermissionValidationResult validateWithDegradation(
        ...
        if (systemLoad < 0.7) {
            ...
            // 负载较高：简化验证（跳过非关键验证器）
            log.warn("系统负载{}，启用简化验证", systemLoad);
            return simplifiedService.validate(request);
            
        } else {
            // 负载极高：紧急模式（仅关键验证）
            log.error("系统负载{}，启用紧急验证模式", systemLoad);
...
public class SimplifiedValidationService {
    ...
    private static final Set<String> CRITICAL_VALIDATORS = Set.of(
        "AccountStatusValidator",    // 账户状态
        "AreaPermissionValidator",    // 区域权限
        "BalanceValidator"            // 余额检查
    );
...
public class EmergencyValidationService {
    ...
    public PermissionValidationResult validate(ValidationContext context) {
        ...
        if (account == null || !account.getAvailable()) {
            return PermissionValidationResult.fail("账户不可用");
        }
        
        if (account.getBalance() <= 0) {
            return PermissionValidationResult.fail("余额不足");
        }
...
---------------------------------------------------------------------------
验证权限       1200000    45      5    200   70    85    120   0.01%    2000
...
**性能瓶颈分析：**
1. ✅ **数据库连接池**：使用率65%，未满载
2. ✅ **Redis连接池**：使用率40%，性能良好
3. ✅ **CPU使用率**：平均55%，有余量
4. ⚠️ **内存使用率**：峰值85%，需监控
5. ✅ **网络IO**：平均300Mbps，未饱和
...
**基础设施：**
- [ ] 服务器资源充足（CPU 8核+、内存 16GB+、磁盘 200GB+）
- [ ] 数据库主从配置完成，读写分离
- [ ] Redis集群部署（3主3从）
- [ ] 负载均衡配置（至少2个应用节点）
- [ ] 网络带宽充足（10Gbps+）
...
**应用配置：**
- [ ] 连接池参数已优化
- [ ] JVM参数已调优（-Xms8g -Xmx8g -XX:+UseG1GC）
- [ ] 日志级别设置为INFO（DEBUG仅在排查问题时使用）
- [ ] 异步线程池配置合理
- [ ] 超时参数已配置
...
**核心更新**：
- ✅ 新增"经营模式与权限验证关系"章节
- ✅ 补充餐别制/超市制/混合模式的差异化验证规则
- ✅ 新增验证规则矩阵流程图
- ✅ 明确不同模式下的适用验证点和不适用验证点

**业务影响**：
- 权限验证系统现在能够根据区域的`manage_mode`字段动态调整验证链
- 餐别制区域强制验证餐别权限和就餐时间
- 超市制区域强制验证商品权限和库存
- 混合模式支持用户选择消费方式并动态验证
