...
flowchart TD
    ...
    B -->|manage_mode=1<br/>餐别制| C1[必须配置餐别]
    B -->|manage_mode=2<br/>超市制| C2[跳过餐别配置]
    B -->|manage_mode=3<br/>混合模式| C3{是否支持定值?}
    
    C1 --> D1[步骤1: 创建餐别分类]
    D1 --> D2[步骤2: 创建具体餐别]
    D2 --> D3[步骤3: 配置时间窗口]
    D3 --> D4[步骤4: 关联区域]
    D4 --> E1[配置完成<br/>✅ 支持定值消费]
    
    C2 --> E2[配置完成<br/>✅ 仅支持商品消费]
    
    C3 -->|是| D1
    C3 -->|否| E3[跳过餐别配置<br/>✅ 仅支持商品消费]
```
...
erDiagram
    ...
    POSID_MEAL_CATEGORY ||--o{ POSID_ACCOUNTKIND_MEAL_CATEGORY : "账户类别关联"
    POSID_ACCOUNTKIND ||--o{ POSID_ACCOUNTKIND_MEAL_CATEGORY : "可用分类"
    POSID_AREA }o--o{ POSID_MEAL_CATEGORY : "提供的分类(JSON)"
    ...
    POSID_MEAL {
        VARCHAR id PK "餐别ID"
        VARCHAR code UK "餐别编号"
        VARCHAR name "餐别名称"
        VARCHAR category_id FK "所属分类ID"
        VARCHAR start_time "开始时间"
        VARCHAR end_time "结束时间"
        DECIMAL base_price "基础价格"
        TEXT price_rules "价格规则JSON"
        INT sort_order "排序"
        BOOLEAN available "是否启用"
        DATETIME create_time "创建时间"
        DATETIME update_time "更新时间"
    ...
    POSID_ACCOUNTKIND_MEAL_CATEGORY {
        VARCHAR id PK "关联ID"
        VARCHAR account_kind_id FK "账户类别ID"
        VARCHAR meal_category_id FK "餐别分类ID"
        BOOLEAN include_all_meals "是否包含分类下所有餐别"
        DATETIME create_time "创建时间"
    }
