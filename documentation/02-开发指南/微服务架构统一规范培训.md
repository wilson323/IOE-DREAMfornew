# IOE-DREAM 微服务架构统一规范培训

> **培训目标**: 确保所有开发团队掌握并遵循统一微服务架构规范
> **培训版本**: v2.0.0 (基于 UNIFIED_MICROSERVICES_STANDARDS.md)
> **培训对象**: 全体开发人员、架构师、技术负责人
> **培训时长**: 4小时 (理论2小时 + 实践2小时)

---

## 📋 培训大纲

### **第一部分: 理论培训 (2小时)**

#### **1.1 统一微服务架构概述 (30分钟)**
- 为什么需要统一架构规范？
- 避免"把微服务和单体架构的代码混了"的重要性
- 统一架构带来的好处
- 违反规范的后果

#### **1.2 核心技术栈统一 (45分钟)**
- **服务注册发现**: Nacos (端口8848) 强制使用
- **API网关**: Spring Cloud Gateway (端口8080) 统一入口
- **服务间通信**: GatewayServiceClient 唯一调用方式
- **依赖注入**: @Resource 强制使用
- **Repository层**: @Repository + @MapperScan 统一规范

#### **1.3 禁止使用的过时技术 (30分钟)**
- ❌ FeignClient → ✅ GatewayServiceClient
- ❌ @Autowired → ✅ @Resource
- ❌ Consul/Eureka → ✅ Nacos
- ❌ HikariCP → ✅ Druid
- ❌ javax.* → ✅ jakarta.*
- ❌ @Repository 和 @Mapper 混用

#### **1.4 配置管理统一 (15分钟)**
- 统一 bootstrap.yml 配置模板
- 统一端口分配规范
- 统一环境变量管理

### **第二部分: 实践培训 (2小时)**

#### **2.1 标准微服务创建 (60分钟)**

**演示1: 创建标准的微服务**
```bash
# 1. 创建微服务目录
mkdir microservices/ioedream-example-service

# 2. 创建标准配置文件
cat > microservices/ioedream-example-service/src/main/resources/bootstrap.yml << 'EOF'
spring:
  application:
    name: ${SERVICE_NAME:ioedream-example-service}
  cloud:
    nacos:
      discovery:
        server-addr: ${NACOS_SERVER_ADDR:127.0.0.1:8848}
        namespace: ${NACOS_NAMESPACE:dev}
        group: ${NACOS_GROUP:IOE-DREAM}
        enabled: true
        register-enabled: true
      config:
        server-addr: ${NACOS_SERVER_ADDR:127.0.0.1:8848}
        namespace: ${NACOS_NAMESPACE:dev}
        group: ${NACOS_GROUP:IOE-DREAM}
        file-extension: yaml

server:
  port: ${SERVER_PORT:8099}
EOF
```

**演示2: 标准启动类**
```java
@SpringBootApplication
@EnableDiscoveryClient
@MapperScan("net.lab1024.sa.example.repository")
public class ExampleServiceApplication {
    public static void main(String[] args) {
        SpringApplication.run(ExampleServiceApplication.class, args);
        log.info("[ExampleServiceApplication] 服务启动完成，已注册到Nacos");
    }
}
```

**演示3: 标准Repository层**
```java
@Repository
public interface ExampleRepository extends BaseMapper<ExampleEntity> {

    @Transactional(readOnly = true)
    List<ExampleEntity> selectByStatus(String status);

    @Transactional
    int insertExample(ExampleEntity entity);
}
```

**演示4: 标准服务类**
```java
@Service
public class ExampleServiceImpl implements ExampleService {

    @Resource
    private ExampleRepository exampleRepository;

    @Resource
    private GatewayServiceClient gatewayServiceClient;

    public void processExample(ExampleDTO dto) {
        // 业务逻辑处理
        ExampleEntity entity = buildEntity(dto);
        exampleRepository.insertExample(entity);

        // 通过网关调用其他服务
        OtherServiceResult result = gatewayServiceClient.callOtherService(
            "/api/v1/other/process", dto, OtherServiceResult.class);
    }
}
```

#### **2.2 GatewayServiceClient 使用 (30分钟)**

**演示: 统一服务调用**
```java
@Component
public class ExampleServiceClient {

    @Resource
    private GatewayServiceClient gatewayServiceClient;

    public ResultVO callAuthService(String path, Object request) {
        return gatewayServiceClient.callAuthService(path, request, ResultVO.class);
    }

    public DeviceVO getDeviceInfo(Long deviceId) {
        return gatewayServiceClient.callDeviceService(
            "/api/v1/devices/" + deviceId, null, DeviceVO.class);
    }
}
```

#### **2.3 错误修正练习 (30分钟)**

**练习1: 修正过时的服务调用**
```java
// ❌ 过时的代码 - 需要学员修正
@FeignClient(name = "auth-service")
public interface AuthServiceClient {
    @GetMapping("/api/users/{id}")
    UserVO getUser(@PathVariable Long id);
}

// ✅ 正确的代码 - 学员练习修正
@Component
public class AuthClient {

    @Resource
    private GatewayServiceClient gatewayServiceClient;

    public UserVO getUser(Long userId) {
        return gatewayServiceClient.callAuthService(
            "/api/v1/users/" + userId, null, UserVO.class);
    }
}
```

**练习2: 修正依赖注入**
```java
// ❌ 过时的代码
@Autowired
private UserService userService;

// ✅ 正确的代码 - 学员练习修正
@Resource
private UserService userService;
```

---

## 📝 培训考核

### **理论考核 (30分钟)**
1. IOE-DREAM项目强制使用什么作为服务注册发现中心？
2. 服务间调用应该使用什么方式？
3. Repository层应该使用什么注解？
4. 依赖注入应该使用什么注解？
5. 数据库连接池应该使用什么技术？

### **实践考核 (30分钟)**
1. 创建一个符合规范的微服务
2. 实现GatewayServiceClient调用
3. 修正给定的过时代码示例

---

## 🔧 开发工具配置
## 📋 IOE-DREAM七微服务架构

**核心架构组成**:
- **Gateway Service (8080)**: API网关
- **Common Service (8088)**: 公共模块微服务
- **DeviceComm Service (8087)**: 设备通讯微服务
- **OA Service (8089)**: OA微服务
- **Access Service (8090)**: 门禁服务
- **Attendance Service (8091)**: 考勤服务
- **Video Service (8092)**: 视频服务
- **Consume Service (8094)**: 消费服务
- **Visitor Service (8095)**: 访客服务

**架构特点**:
- 基于Spring Boot 3.5.8 + Java 17
- 严格遵循企业级微服务规范
- 支持高并发、高可用、水平扩展

**技术栈标准**:
- **数据库**: MySQL 8.0 + Druid连接池
- **缓存**: Redis + Caffeine多级缓存
- **注册中心**: Nacos
- **配置中心**: Nacos Config
- **认证授权**: Sa-Token

## 🏗️ 四层架构规范

**标准架构模式**:
```
Controller (接口控制层)
    ↓
Service (核心业务层)
    ↓
Manager (流程管理层)
    ↓
DAO (数据访问层)
```

**层级职责**:
- **Controller层**: HTTP请求处理、参数验证、权限控制
- **Service层**: 核心业务逻辑、事务管理、业务规则验证
- **Manager层**: 复杂流程编排、多数据组装、第三方服务集成
- **DAO层**: 数据库CRUD操作、SQL查询实现、数据访问边界

**严格禁止跨层访问**: Controller不能直接调用Manager/DAO！
### **IDEA 配置**
## ⚠️ IOE-DREAM零容忍规则（强制执行）

**必须遵守的架构规则**:
- ✅ **必须使用 @Resource 注入依赖**
- ✅ **必须使用 @Mapper 注解** (禁止@Repository)
- ✅ **必须使用 Dao 后缀** (禁止Repository)
- ✅ **必须使用 @RestController 注解**
- ✅ **必须使用 @Valid 参数校验**
- ✅ **必须返回统一ResponseDTO格式**
- ✅ **必须遵循四层架构边界**

**严格禁止事项**:
- ❌ **禁止使用 @Autowired 注入**
- ❌ **禁止使用 @Repository 注解**
- ❌ **禁止使用 Repository 后缀命名**
- ❌ **禁止跨层访问**
- ❌ **禁止在Controller中包含业务逻辑**
- ❌ **禁止直接访问数据库**

**违规后果**: P0级问题，立即修复，禁止合并！
- **代码模板**: 配置GatewayServiceClient使用模板
- **实时检查**: 配置过时规范实时提醒
- **代码格式化**: 统一代码格式规范

### **Git Hook 配置**
```bash
#!/bin/bash
# .git/hooks/pre-commit
# 提交前检查

# 检查过时规范
if grep -r "@Autowired\|javax\.\|FeignClient\|consul:\|eureka:\|hikari:" src/main/java; then
    echo "❌ 发现过时规范，请先修正"
    exit 1
fi
```

### **Gradle 插件配置**
```gradle
// 架构合规性检查插件
task architectureComplianceCheck(type: JavaExec) {
    classpath = sourceSets.main.runtimeClasspath
    mainClass = 'com.ioedream.architecture.ComplianceChecker'
}
```

---

## 📚 参考文档

1. **UNIFIED_MICROSERVICES_STANDARDS.md** - 统一微服务架构规范
2. **门禁服务架构混合问题识别报告.md** - 架构问题分析
3. **门禁服务微服务拆分实施方案.md** - 实施方案
4. **代码质量铁血军规.md** - 代码质量标准

---

## 🎯 培训要求

### **出勤要求**
- 全体开发人员必须参加
- 理论和实践课程都需要完成
- 考核必须通过

### **考核标准**
- 理论考核: 80分以上
- 实践考核: 100%符合规范
- 代码审查: 无过时规范使用

### **持续跟进**
- 每周代码审查检查规范遵循情况
- CI/CD自动检查违规使用
- 月度架构合规性报告

---

**培训负责人**: 架构组
**技术支持**: DevOps团队
**培训时间**: 2025-12-01 开始