...
### 🚫 绝对禁止 (按照CLAUDE.md v4.0.0标准)
```markdown
❌ 禁止使用 @Autowired 注入，必须使用 @Resource
❌ 禁止使用构造函数注入，必须使用 @Resource
❌ 禁止使用魔法数字，必须定义为常量
...
✅ 必须严格遵循四层架构规范：Controller → Service → Manager → DAO
✅ 必须使用 @Mapper 注解，禁止@Repository
✅ 必须使用 Dao 后缀命名，禁止 Repository 后缀
✅ 必须继承 BaseMapper<Entity>
✅ 必须代码覆盖率 ≥ 80%
✅ 必须使用 Jakarta EE 3.0+ 包名，禁止 javax 包名
```
