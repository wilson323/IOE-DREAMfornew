# IOE-DREAM 智能管理系统开发指南

> **📋 版本**: v2.0.0
> **📋 创建时间**: 2025-01-10
> **📋 最后更新**: 2025-11-27
> **📋 说明**: 本指南为IOE-DREAM智能管理系统开发的快速参考和最佳实践，支持双轨制架构（单体 + 微服务）
...
### 架构概述
IOE-DREAM采用**渐进式微服务重构**策略，同时维护两套架构：
...
│   ├── ioedream-identity-service (身份服务:8082)
│   ├── ioedream-device-service (设备服务:8083)
│   └── ioedream-area-service (区域服务:8084)
├── 业务服务层
...
```yaml
# application.yml 标准配置模板
server:
  port: 808x  # 服务端口

spring:
  application:
    name: ioedream-{service}-service
  profiles:
    active: dev
  cloud:
    nacos:
  ...
  datasource:
    ...
    url: jdbc:mysql://localhost:3306/ioedream_{service}?useUnicode=true&characterEncoding=utf-8
...
```java
// Feign客户端接口定义
@FeignClient(name = "ioedream-{target-service}-service",
             contextId = "{service}FeignClient",
             fallback = {TargetService}FeignClientFallback.class)
public interface {TargetService}FeignClient {

    @GetMapping("/api/{target-service}/health")
    ResponseDTO<String> health();

    @PostMapping("/api/{target-service}/data/sync")
    ResponseDTO<Void> syncData(@RequestBody DataSyncDTO syncDTO);
}

// 熔断降级实现
@Component
public class {TargetService}FeignClientFallback implements {TargetService}FeignClient {

    @Override
    public ResponseDTO<String> health() {
        log.warn("{target-service}服务不可用，返回降级响应");
        return ResponseDTO.userError("服务暂时不可用");
    }

    @Override
    public ResponseDTO<Void> syncData(@RequestBody DataSyncDTO syncDTO) {
        log.warn("{target-service}服务不可用，数据同步任务将稍后重试");
        return ResponseDTO.userError("数据同步服务暂时不可用");
    }
}
```
...
```markdown
🔴 强制要求：
□ 所有类必须使用 @Resource 注入，禁止 @Resource
...
#### 设计理念
IOE-DREAM项目采用以人为中心的统一生物特征管理架构，解决生物特征数据分散在各业务模块的问题，实现"设备下发生物特征模板时可以统一从同一处获取，且生物特征应该是每个人对应的生物特征"的核心需求。
...
**2. 生物特征缓存管理器 (BiometricCacheManager)**
```java
// 多级缓存策略：L1 Caffeine + L2 Redis
@Component
...
#### 设计原则
基于业务独立性原则，采用分层式设备适配器架构：基础接口定义在sa-base模块，协议实现由各业务模块自行管理。
...
└── 视频监控模块
    └── VideoDeviceAdapter.java         # 视频设备适配器
```

#### 基础接口定义
```java
// 位置：sa-base/src/main/java/net/lab1024/sa/base/common/device/
...
#### 业务模块适配器示例
```java
// 位置：sa-admin/src/main/java/net/lab1024/sa/admin/module/access/adapter/
...
public class AccessDeviceAdapter implements DeviceAdapterInterface {
    ...
    public DeviceDispatchResult remoteOpenDoor(SmartDeviceEntity device, String doorId) {
        AccessProtocolInterface adapter = getProtocolAdapter(device);
        return adapter.remoteOpenDoor(device, doorId);
    }

    // 门禁特有功能：获取门禁记录
    public List<Map<String, Object>> getAccessRecords(SmartDeviceEntity device,
                                                      String startTime, String endTime, Integer recordCount) {
        AccessProtocolInterface adapter = getProtocolAdapter(device);
...
#### 设计目标
将区域管理从各业务模块中提取出来，作为基础模块统一管理，支持人员与区域的关联以及基于区域的设备下发策略。

#### 核心组件

**1. 基础区域管理模块**
```java
// 位置：sa-base/src/main/java/net/lab1024/sa/base/module/area/
@Entity
@Table("t_base_area")
public class AreaEntity extends BaseEntity {
    ...
    private String areaType;      // 区域类型：DOOR, BUILDING, FLOOR, ROOM
    private Long parentId;        // 父区域ID
    private String extension;     // 扩展配置JSON
}
...
public class AreaService {
    ...
    // 获取区域设备
    public List<SmartDeviceEntity> getAreaDevices(Long areaId, String deviceType);

    // 区域设备下发
    public DeviceDispatchResult dispatchToAreaDevices(Long areaId, Map<String, Object> data);
}
```
...
public class PersonAreaRelationEntity extends BaseEntity {
    ...
    private Long areaId;
    private String relationType;  // 关系类型：MAIN, SECONDARY, TEMPORARY
    private Date validFrom;       // 生效时间
    private Date validTo;         // 失效时间
}

// 人员区域管理服务
@Service
public class PersonAreaService {
    // 添加人员区域关联
    public void addPersonAreaRelation(PersonAreaRelationDTO relationDTO);

    // 获取人员有效区域
    public List<AreaEntity> getPersonValidAreas(Long personId);

    // 基于区域策略下发设备数据
    public DeviceDispatchResult dispatchByAreaStrategy(Long personId, Map<String, Object> data);
}
```
...
public class DeviceDispatchStrategyEngine {
    ...
    // 执行智能下发策略
    public Map<String, DeviceDispatchResult> executeDispatchStrategy(Long personId,
                                                                   Map<String, Object> data,
                                                                   String businessType);

    // 设备下发策略配置
    public void updateDispatchStrategy(DispatchStrategyConfig config);
}
```
...
sequenceDiagram
    ...
    participant UBE as 统一生物特征引擎
    participant BCM as 生物特征缓存
    participant ASE as 区域策略引擎
    participant ADAPTER as 设备适配器
    ...
    loop 遍历目标设备
        UBE->>ADAPTER: 调用业务模块适配器
        ADAPTER->>Device: 协议通信
        Device-->>ADAPTER: 返回执行结果
...
public class BiometricEncryptionService {
    ...
    public String decryptBiometricTemplate(String encryptedData) {
        return SM4Util.decrypt(encryptedData, secretKey);
    }
}
```
...
public class BiometricController {
    ...
    public ResponseDTO<String> dispatchBiometric(@Valid @RequestBody BiometricDispatchDTO dispatchDTO) {
        // 统一下发逻辑
        return ResponseDTO.ok(biometricService.dispatch(dispatchDTO));
    }
}
```
...
```java
@Component
public class BiometricAuditLogger {

    @EventListener
    public void handleBiometricDispatchEvent(BiometricDispatchEvent event) {
        // 记录详细的操作审计日志
        AuditLogEntity auditLog = AuditLogEntity.builder()
            .operationType("BIOMETRIC_DISPATCH")
            .targetId(event.getPersonId())
            .operationResult(event.isSuccess() ? "SUCCESS" : "FAILURE")
            .operationDetail(buildOperationDetail(event))
            .build();
...
- [消费系统开发检查清单](./CHECKLISTS/消费系统开发检查清单.md)
- [考勤系统开发检查清单](./CHECKLISTS/考勤系统开发检查清单.md)
- [智能视频系统开发检查清单](./CHECKLISTS/智能视频系统开发检查清单.md)

### 技术规范文档
- [技术迁移规范](./TECHNOLOGY_MIGRATION.md)
...
- **安全问题**: 联系安全团队
- **运维问题**: 联系运维团队
- **生物特征和设备适配器问题**: 参考统一生物特征架构文档

---

*最后更新：2025-11-24*
*维护者：SmartAdmin开发团队*
*新增内容：统一生物特征管理架构、分层式设备适配器架构、统一区域管理架构*