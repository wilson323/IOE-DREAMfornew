...
**软件端职责**:
```
接收请求（pin + verifytype） → 验证用户权限
→ 验证认证方式是否允许 → 反潜/互锁/时间段验证
→ 返回开门指令
```
...
sequenceDiagram
    ...
    Access->>MultiModal: authenticate(request)
    Note right of MultiModal: 验证用户是否允许使用该认证方式
    MultiModal->>Access: 认证方式验证结果
    Access->>Manager: verifyAntiPassback() + verifyInterlock() + ...
...
public interface MultiModalAuthenticationStrategy {
     ...
     * 验证用户是否允许使用该认证方式
     * <p>
     * ⚠️ 注意：不是进行人员识别，设备端已经识别了人员并发送了pin
     * </p>
     * <p>
     * 验证内容：
     * 1. 用户权限配置中是否允许该认证方式
     ...
     * </p>
     *
     * @param request 验证请求（包含userId、verifyType等）
     * @return 认证方式验证结果
     */
