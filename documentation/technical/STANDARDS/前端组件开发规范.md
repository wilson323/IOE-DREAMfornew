# 前端组件开发规范

## 📋 文档说明

本文档规定SmartAdmin前端项目（Vue 3 + JavaScript）中组件开发的统一规范。

**强制执行**：所有前端组件开发必须遵守本规范。

---

## 📂 目录结构规范

### 标准目录结构
```
smart-admin-web-javascript/
└── src/
    ├── api/                    # API接口封装
    │   ├── system/            # 系统模块API
    │   │   ├── area-api.js
    │   │   ├── menu-api.js
    │   │   └── employee-api.js
    │   └── support/           # 支持模块API
    │       ├── dict-api.js
    │       └── config-api.js
    ├── views/                  # 页面组件
    │   ├── system/            # 系统模块页面
    │   │   ├── area/
    │   │   │   ├── index.vue        # 主页面
    │   │   │   └── area-modal.vue   # 弹窗组件
    │   │   ├── menu/
    │   │   │   └── menu-list.vue    # 主页面（历史命名）
    │   │   └── employee/
    │   │       ├── index.vue
    │   │       └── employee-form.vue
    │   └── support/           # 支持模块页面
    │       └── dict/
    │           ├── index.vue
    │           └── dict-data-modal.vue
    ├── components/            # 公共组件
    ├── store/                 # Pinia状态管理
    └── utils/                 # 工具函数
```

---

## 📝 文件命名规范

### 主列表页面

#### 推荐命名：`index.vue`
```
✅ 推荐（新开发统一使用）
/views/system/area/index.vue
/views/system/employee/index.vue
/views/support/dict/index.vue
```

**优点**：
- 路径简洁：`/views/system/area/`即可定位
- 符合Vue生态惯例
- 便于统一管理

#### 备选命名：`模块名-list.vue`
```
✅ 可接受（历史遗留）
/views/system/menu/menu-list.vue
/views/system/role/role-list.vue
```

**使用场景**：
- 仅用于兼容历史代码
- 新开发不推荐使用
- 一个目录下有多个列表页面时可使用

### 弹窗组件

**格式**：`{module}-modal.vue`

```
✅ 正确
/views/system/area/area-modal.vue
/views/system/employee/employee-modal.vue
/views/support/dict/dict-data-modal.vue
```

**命名规则**：
- 使用 `-modal` 后缀
- 与功能模块名称保持一致
- 小写字母，用连字符分隔

### 表单组件

**格式**：`{module}-form.vue`

```
✅ 正确
/views/system/employee/employee-form.vue
/views/system/department/department-form.vue
```

### 详情页面

**格式**：`{module}-detail.vue`

```
✅ 正确
/views/system/order/order-detail.vue
/views/system/employee/employee-detail.vue
```

---

## 🗂️ API封装规范

### API文件命名

**格式**：`{module}-api.js`

```
✅ 正确
/api/system/area-api.js
/api/system/employee-api.js
/api/support/dict-api.js
```

### API接口定义

**标准模板**：
```javascript
/*
 * {模块}管理API
 *
 * @Author:    开发者
 * @Date:      2025-01-10
 * @Copyright  SmartAdmin
 */
import { getRequest, postRequest } from '/@/lib/axios';

export const {module}Api = {
  /**
   * 分页查询
   */
  queryPage: (param) => {
    return postRequest('/{module}/query', param);
  },

  /**
   * 获取详情
   */
  getDetail: (id) => {
    return getRequest(`/{module}/detail/${id}`);
  },

  /**
   * 新增
   */
  add: (param) => {
    return postRequest('/{module}/add', param);
  },

  /**
   * 更新
   */
  update: (param) => {
    return postRequest('/{module}/update', param);
  },

  /**
   * 删除
   */
  delete: (id) => {
    return postRequest(`/{module}/delete/${id}`);
  },

  /**
   * 批量删除
   */
  batchDelete: (ids) => {
    return postRequest('/{module}/batchDelete', ids);
  },
};
```

**要点说明**：
- 导出对象命名：`{module}Api`
- 每个方法添加注释说明
- 路径与后端Controller保持一致
- 使用合适的HTTP方法（getRequest/postRequest）

---

## 🎨 组件开发规范

### 主列表页面结构

**标准模板**（index.vue）：
```vue
<!--
  * {模块}管理
  *
  * @Author:    开发者
  * @Date:      2025-01-10
  * @Copyright  SmartAdmin
-->
<template>
  <div class="{module}-manage">
    <!-- 1. 搜索区域 -->
    <a-card :bordered="false" size="small" class="search-card">
      <a-form layout="inline" :model="queryForm">
        <a-form-item label="关键字">
          <a-input v-model:value="queryForm.keywords" placeholder="请输入关键字" />
        </a-form-item>
        <a-form-item>
          <a-space>
            <a-button type="primary" @click="handleSearch">查询</a-button>
            <a-button @click="handleReset">重置</a-button>
          </a-space>
        </a-form-item>
      </a-form>
    </a-card>

    <!-- 2. 操作按钮区域 -->
    <a-card :bordered="false" size="small" class="table-card">
      <div class="table-header">
        <a-space>
          <a-button type="primary" @click="handleAdd" v-privilege="'system:{module}:add'">
            <template #icon><PlusOutlined /></template>
            新增
          </a-button>
        </a-space>
      </div>

      <!-- 3. 数据表格 -->
      <a-table
        :columns="columns"
        :data-source="tableData"
        :loading="loading"
        :pagination="false"
        row-key="id"
      >
        <template #bodyCell="{ column, record }">
          <!-- 操作列 -->
          <template v-if="column.key === 'action'">
            <a-space>
              <a-button type="link" size="small" @click="handleEdit(record)" v-privilege="'system:{module}:update'">
                编辑
              </a-button>
              <a-popconfirm title="确定删除？" @confirm="handleDelete(record.id)" v-privilege="'system:{module}:delete'">
                <a-button type="link" size="small" danger>删除</a-button>
              </a-popconfirm>
            </a-space>
          </template>
        </template>
      </a-table>

      <!-- 4. 分页 -->
      <div class="smart-query-table-page">
        <a-pagination
          v-model:current="queryForm.pageNum"
          v-model:pageSize="queryForm.pageSize"
          :total="total"
          @change="loadData"
        />
      </div>
    </a-card>

    <!-- 5. 弹窗组件 -->
    <XxxModal
      v-model:visible="modalVisible"
      :data="currentData"
      @success="handleModalSuccess"
    />
  </div>
</template>

<script setup>
import { ref, reactive, onMounted } from 'vue';
import { message } from 'ant-design-vue';
import { PlusOutlined } from '@ant-design/icons-vue';
import { {module}Api } from '/@/api/system/{module}-api';
import XxxModal from './{module}-modal.vue';

// 响应式数据
const loading = ref(false);
const tableData = ref([]);
const total = ref(0);
const modalVisible = ref(false);
const currentData = ref(null);

// 查询表单
const queryForm = reactive({
  keywords: '',
  pageNum: 1,
  pageSize: 10,
});

// 表格列定义
const columns = [
  { title: '名称', dataIndex: 'name', key: 'name' },
  { title: '操作', key: 'action', width: 200, fixed: 'right' },
];

// 方法定义
const loadData = async () => {
  try {
    loading.value = true;
    const res = await {module}Api.queryPage(queryForm);
    if (res.data) {
      tableData.value = res.data.list;
      total.value = res.data.total;
    }
  } catch (error) {
    message.error('加载失败');
  } finally {
    loading.value = false;
  }
};

const handleSearch = () => {
  queryForm.pageNum = 1;
  loadData();
};

const handleReset = () => {
  queryForm.keywords = '';
  queryForm.pageNum = 1;
  loadData();
};

const handleAdd = () => {
  currentData.value = null;
  modalVisible.value = true;
};

const handleEdit = (record) => {
  currentData.value = { ...record };
  modalVisible.value = true;
};

const handleDelete = async (id) => {
  try {
    await {module}Api.delete(id);
    message.success('删除成功');
    loadData();
  } catch (error) {
    message.error(error.msg || '删除失败');
  }
};

const handleModalSuccess = () => {
  modalVisible.value = false;
  loadData();
};

// 生命周期
onMounted(() => {
  loadData();
});
</script>

<style lang="less" scoped>
.{module}-manage {
  padding: 16px;

  .search-card {
    margin-bottom: 16px;
  }

  .table-card {
    .table-header {
      margin-bottom: 16px;
    }
  }
}
</style>
```

---

### 弹窗组件结构

**标准模板**（{module}-modal.vue）：
```vue
<!--
  * {模块}新增/编辑弹窗
  *
  * @Author:    开发者
  * @Date:      2025-01-10
  * @Copyright  SmartAdmin
-->
<template>
  <a-modal
    :visible="visible"
    :title="modalTitle"
    :width="800"
    :confirm-loading="confirmLoading"
    @ok="handleSubmit"
    @cancel="handleCancel"
  >
    <a-form
      ref="formRef"
      :model="formData"
      :rules="formRules"
      :label-col="{ span: 6 }"
      :wrapper-col="{ span: 16 }"
    >
      <a-form-item label="名称" name="name">
        <a-input v-model:value="formData.name" placeholder="请输入名称" />
      </a-form-item>
      <!-- 其他表单项 -->
    </a-form>
  </a-modal>
</template>

<script setup>
import { ref, reactive, computed, watch } from 'vue';
import { message } from 'ant-design-vue';
import { {module}Api } from '/@/api/system/{module}-api';

// Props
const props = defineProps({
  visible: {
    type: Boolean,
    default: false,
  },
  data: {
    type: Object,
    default: null,
  },
});

// Emits
const emit = defineEmits(['update:visible', 'success']);

// 响应式数据
const formRef = ref();
const confirmLoading = ref(false);

const formData = reactive({
  id: undefined,
  name: '',
});

// 表单验证规则
const formRules = {
  name: [
    { required: true, message: '请输入名称', trigger: 'blur' },
  ],
};

// 计算属性
const modalTitle = computed(() => {
  return formData.id ? '编辑' : '新增';
});

// 方法定义（在watch之前）
const resetForm = () => {
  Object.assign(formData, {
    id: undefined,
    name: '',
  });
  formRef.value?.clearValidate();
};

// 监听数据变化
watch(
  () => props.data,
  (newVal) => {
    if (newVal) {
      Object.assign(formData, { ...newVal });
    } else {
      resetForm();
    }
  },
  { immediate: true }
);

const handleSubmit = async () => {
  try {
    await formRef.value.validate();
    confirmLoading.value = true;

    if (formData.id) {
      await {module}Api.update(formData);
      message.success('更新成功');
    } else {
      await {module}Api.add(formData);
      message.success('新增成功');
    }

    emit('success');
    handleCancel();
  } catch (error) {
    if (error.errorFields) return;
    message.error(error.msg || '操作失败');
  } finally {
    confirmLoading.value = false;
  }
};

const handleCancel = () => {
  emit('update:visible', false);
  resetForm();
};
</script>
```

---

## 🔑 权限控制规范

### 使用v-privilege指令

**格式**：`v-privilege="'system:{module}:{action}'"`

```vue
<!-- ✅ 正确：完整的权限格式 -->
<a-button type="primary" v-privilege="'system:area:add'">新增</a-button>
<a-button v-privilege="'system:area:update'">编辑</a-button>
<a-button v-privilege="'system:area:delete'">删除</a-button>

<!-- ❌ 错误：缺少domain前缀 -->
<a-button v-privilege="'area:add'">新增</a-button>

<!-- ❌ 错误：使用中文 -->
<a-button v-privilege="'system:区域:新增'">新增</a-button>
```

### 权限与后端一致性

**后端Controller**：
```java
@SaCheckPermission("system:area:add")
```

**前端组件**：
```vue
v-privilege="'system:area:add'"
```

**数据库配置**：
```sql
api_perms = 'system:area:add'
web_perms = 'system:area:add'
```

---

## 🗄️ 数据库菜单配置

### 菜单表配置规范

```sql
INSERT INTO t_menu (
  menu_name,
  menu_type,
  parent_id,
  path,
  component,
  perms_type,
  api_perms,
  web_perms,
  icon,
  sort,
  cache_flag,
  visible_flag,
  disabled_flag,
  deleted_flag,
  create_user_id,
  create_time
) VALUES (
  '区域设置',           -- 菜单名称
  2,                   -- 菜单类型：1=目录 2=菜单 3=按钮
  50,                  -- 父菜单ID
  '/setting/area',     -- 路由路径
  '/system/area/index.vue',  -- ✅ 组件路径必须与实际文件一致
  1,                   -- 权限类型
  NULL,                -- API权限（菜单级别为NULL）
  NULL,                -- WEB权限（菜单级别为NULL）
  'EnvironmentOutlined',  -- 图标
  2,                   -- 排序
  1,                   -- 缓存标识
  1,                   -- 可见标识
  0,                   -- 禁用标识
  0,                   -- 删除标识
  1,                   -- 创建人
  NOW()                -- 创建时间
);
```

### 组件路径格式

**规范**：`/{domain}/{module}/index.vue` 或 `/{domain}/{module}/{module}-list.vue`

```sql
-- ✅ 正确：路径与实际文件一致
component = '/system/area/index.vue'         -- 文件：/views/system/area/index.vue
component = '/system/menu/menu-list.vue'     -- 文件：/views/system/menu/menu-list.vue
component = '/support/dict/index.vue'        -- 文件：/views/support/dict/index.vue

-- ❌ 错误：路径与实际文件不一致
component = '/system/area/area-list.vue'     -- 实际文件是index.vue
component = '/system/area.vue'               -- 缺少目录层级
```

---

## ⚠️ 常见错误与修正

### 错误1：组件路径配置错误

```sql
-- ❌ 错误：数据库配置的组件路径不存在
INSERT INTO t_menu (component) VALUES ('/system/area/area-list.vue');
-- 实际文件：/views/system/area/index.vue

-- ✅ 修正：确保路径与实际文件一致
INSERT INTO t_menu (component) VALUES ('/system/area/index.vue');
```

### 错误2：权限指令格式错误

```vue
<!-- ❌ 错误：缺少domain前缀 -->
<a-button v-privilege="'area:add'">新增</a-button>

<!-- ✅ 修正：使用完整格式 -->
<a-button v-privilege="'system:area:add'">新增</a-button>
```

### 错误3：API路径与后端不一致

```javascript
// 后端：/area/query
// ❌ 错误：前端路径不一致
export const areaApi = {
  queryPage: (param) => postRequest('/system/area/page', param),
};

// ✅ 修正：与后端保持一致
export const areaApi = {
  queryPage: (param) => postRequest('/area/query', param),
};
```

### 错误4：Vue 3声明顺序错误

```vue
<script setup>
// ❌ 错误：watch在函数之前执行
watch(() => props.data, () => {
  resetForm();  // ReferenceError
}, { immediate: true });

const resetForm = () => { ... };

// ✅ 修正：函数在watch之前声明
const resetForm = () => { ... };

watch(() => props.data, () => {
  resetForm();  // ✅ 正确
}, { immediate: true });
</script>
```

---

## 🎓 最佳实践建议

1. **统一使用index.vue**
   - 新开发的模块统一使用`index.vue`作为主页面
   - 保持代码风格一致

2. **权限指令必须完整**
   - 始终使用`system:{module}:{action}`格式
   - 与后端Controller保持一致

3. **API封装独立管理**
   - 每个模块独立API文件
   - 方法命名清晰、注释完整

4. **组件职责单一**
   - 主列表页面：负责数据展示和操作
   - 弹窗组件：负责表单录入
   - 避免一个组件承担过多职责

5. **样式使用scoped**
   - 所有组件样式使用`<style scoped>`
   - 避免样式污染

6. **参考现有组件**
   - 新开发功能前，先查找类似功能
   - 复制参考组件结构，保持一致性

---

## 📖 相关文档

- [权限命名规范](./权限命名规范.md)
- [API路径规范](./API路径规范.md)
- [Vue3 Composition API最佳实践](./Vue3_Composition_API最佳实践.md)
- [AI辅助开发检查清单](../AI_DEV_CHECKLIST.md)

---

**版本**：v1.0
**更新时间**：2025-11-11
**维护人**：开发团队
