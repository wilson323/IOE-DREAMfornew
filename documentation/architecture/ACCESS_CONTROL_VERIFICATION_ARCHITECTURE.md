...
sequenceDiagram
    ...
    门禁设备->>门禁设备: 1. 采集生物特征
    门禁设备->>门禁设备: 2. 本地识别匹配userId
    门禁设备->>设备通讯服务: 3. 发送验证请求<br/>POST /iclock/cdata?SN=xxx&AuthType=device
    Note over 门禁设备,设备通讯服务: 设备已识别用户身份<br/>但未验证权限逻辑
    设备通讯服务->>门禁服务: 4. 转发验证请求
    门禁服务->>权限验证: 5. 验证权限逻辑
    Note over 门禁服务,权限验证: 检查:<br/>- 反潜规则<br/>- 时间段权限<br/>- 黑名单<br/>- 门禁级别
    权限验证-->>门禁服务: 6. 返回验证结果
    门禁服务-->>设备通讯服务: 7. 返回结果+控制指令
    设备通讯服务-->>门禁设备: 8. HTTP 200 OK<br/>AUTH=SUCCEED/FAILED
    alt 验证成功
        门禁设备->>门禁设备: 9. 开门(继电器)
    else 验证失败
        门禁设备->>门禁设备: 9. 拒绝通行(语音提示)
    end
    门禁设备->>设备通讯服务: 10. 上传通行记录
```
