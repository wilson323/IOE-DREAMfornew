...
```mermaid
classDiagram
    class GlobalThreadPoolConfiguration {
        -int CPU_COUNT
        +coreExecutor() ThreadPoolTaskExecutor
        +ioExecutor() ThreadPoolTaskExecutor
        +scheduledExecutor() ThreadPoolTaskExecutor
    }
    ...
    GlobalThreadPoolConfiguration --> ThreadPoolTaskExecutor
```
...
| 方法类型 | 注解配置 | 说明 |
|---------|---------|------|
| 纯查询 | `@Transactional(readOnly = true)` | 减少锁竞争 |
| 写入操作 | `@Transactional(rollbackFor = Exception.class)` | 支持回滚 |
...
| 资源优化指南 | `documentation/technical/RESOURCE_OPTIMIZATION_GUIDE.md` | 性能优化 |
...
| v2.1.0 | 2025-12-17 | 新增性能优化详细设计 | 架构组 |
