...
<template>
  <div>
    <a-popover :open="visible" @update:open="val => emit('update:visible', val)" placement="bottomLeft" trigger="click">
      <template #title>
        <a-form-item>
          <a-radio-group @change="updateSelectIconArray" :value="iconStyle" @update:value="val => emit('update:value', val)" style="margin: 8px">
        ...
        <a-form-item>
          <a-input-search :value="searchValue" @update:value="val => emit('update:value', val)" placeholder="输入英文关键词进行搜索" @change="updateSelectIconArray" />
        </a-form-item>
      </template>
