...
<script setup>
  ...
  function onOpenChange(openKeysParams) {
    ...
    // 获取当前选中菜单的父级菜单ID
    let currentParentKeys = parentList.map(item => Number(item.name));

    // 如果新展开的菜单中包含当前选中菜单的父级，则只保留这些
    const filteredOpenKeys = openKeysParams.filter(key =>
      currentParentKeys.includes(key) || rootSubmenuKeys.value.indexOf(key) === -1
    );

    openKeys.value = filteredOpenKeys;
  }
  defineExpose({
    updateOpenKeysAndSelectKeys,
  });
