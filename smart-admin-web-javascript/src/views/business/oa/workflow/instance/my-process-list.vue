...
<template>
  <div class="my-process-list-page">
    <!-- 查询表单区域 -->
    ...
    <!-- 流程实例列表区域 -->
    <a-card size="small" :bordered="false" class="process-list-card">
      <a-table
        row-key="instanceId"
...
<script setup>
  ...
  async function queryProcessList() {
    try {
      ...
      await workflowStore.fetchMyProcessList(params);
      processList.value = workflowStore.myProcessList;
      total.value = workflowStore.myProcessTotal;
    } catch (err) {
      console.error('查询我发起的流程列表失败:', err);
      smartSentry.captureError(err);
      message.error('查询我发起的流程列表失败');
    } finally {
      loading.value = false;
    }
  }
