...
-->
<template>
  <a-modal :open="visible" @update:open="val => emit('update:visible', val)" :title="formState.departmentId ? '编辑部门' : '添加部门'" @ok="handleOk" destroyOnClose>
    <a-form ref="formRef" :model="formState" :rules="rules" layout="vertical">
      <a-form-item label="上级部门" name="parentId" v-if="formState.parentId != 0">
        <DepartmentTreeSelect ref="departmentTreeSelect" :value="formState.parentId" @update:value="val => emit('update:value', val)" :defaultValueFlag="false"
          width="100%" />
      </a-form-item>
      ...
      <a-form-item label="部门负责人" name="managerId">
        <EmployeeSelect ref="employeeSelect" placeholder="请选择部门负责人" width="100%" :value="formState.managerId" @update:value="val => emit('update:value', val)"
          :leaveFlag="false" />
      </a-form-item>
      <a-form-item label="部门排序 （值越大越靠前！）" name="sort">
        <a-input-number style="width: 100%" :value="formState.sort" @update:value="val => emit('update:value', val)" :min="0" placeholder="请输入部门排序" />
      </a-form-item>
    </a-form>
  </a-modal>
</template>
