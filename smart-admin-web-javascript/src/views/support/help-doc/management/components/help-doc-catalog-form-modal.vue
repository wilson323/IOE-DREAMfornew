...
-->
<template>
  <a-modal :open="visible" @update:open="val => emit('update:visible', val)" :title="formState.helpDocCatalogId ? '编辑目录' : '添加目录'" @ok="handleOk" destroyOnClose>
    <a-form ref="formRef" :model="formState" :rules="rules" layout="vertical">
      <a-form-item label="上级目录" name="parentId" v-if="formState.parentId !== 0">
        <HelpDocCatalogTreeSelect ref="helpDocCatalogTreeSelect" :value="formState.parentId" @update:value="val => emit('update:value', val)" :defaultValueFlag="false" width="100%" />
      </a-form-item>
      ...
      <a-form-item label="目录排序 （值越小越靠前！）" name="sort">
        <a-input-number style="width: 100%" :value="formState.sort" @update:value="val => emit('update:value', val)" :min="0" placeholder="请输入目录名称" />
      </a-form-item>
    </a-form>
  </a-modal>
</template>
