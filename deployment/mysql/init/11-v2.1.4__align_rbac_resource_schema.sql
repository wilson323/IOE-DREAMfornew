...
-- 背景: V2.1.3 将 t_common_permission 重命名为 t_rbac_resource，但字段仍是 permission 模型
-- 目标实体: microservices-common/net.lab1024.sa.common.rbac.domain.entity.RbacResourceEntity
-- =====================================================

USE ioedream;

-- 1) 确保 t_rbac_resource 存在（若不存在则创建符合实体的结构）
...
-- 3) 一次性数据迁移（仅在新字段为空时填充）
-- permission_code -> resource_code
...
-- 4) 确保索引存在（MySQL 8 支持 IF NOT EXISTS）
