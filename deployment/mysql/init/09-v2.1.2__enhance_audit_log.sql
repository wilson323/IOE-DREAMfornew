...
-- IOE-DREAM 数据库迁移脚本
-- 版本: V2.1.2
-- 描述: 对齐 t_audit_log 表结构到 AuditLogEntity（增量补字段/索引，幂等执行）
-- 约定: 表命名以 t_* 为准
-- =====================================================

USE ioedream;

-- 确保表存在（若不存在则创建一个最小可用表结构；正常情况下由01-ioedream-schema.sql创建）
...
-- 统一字段补齐：根据AuditLogEntity字段扩展
ALTER TABLE t_audit_log
    -- 兼容老字段：operation/method/params/time/error_msg
...
-- 如需迁移历史数据，可追加UPDATE语句（默认不启用，避免重复覆盖）
-- UPDATE t_audit_log SET operation_desc = COALESCE(operation_desc, operation) WHERE operation_desc IS NULL;
-- UPDATE t_audit_log SET request_method = COALESCE(request_method, method) WHERE request_method IS NULL;
-- UPDATE t_audit_log SET request_params = COALESCE(request_params, params) WHERE request_params IS NULL;
-- UPDATE t_audit_log SET execution_time = COALESCE(execution_time, time) WHERE execution_time IS NULL;
-- UPDATE t_audit_log SET error_message = COALESCE(error_message, error_msg) WHERE error_message IS NULL;
