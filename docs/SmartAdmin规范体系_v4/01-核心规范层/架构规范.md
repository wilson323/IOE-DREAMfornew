...
> **📋 文档版本**: v4.0.0 (整合版)
> **📋 文档职责**: SmartAdmin项目的唯一架构规范权威来源，整合了v1项目管理精华、v2实践经验和v3架构优势。
>
> **🔗 引用关系**: 本文档被[AI开发指令集](../03-AI专用层/AI开发指令集.md)、[测试实施指南](../02-实施指南层/测试实施指南.md)、[安全规范](./安全规范.md)等文档引用。
...
public class UserManager {
     ...
     * 批量获取用户列表（缓存优化）
     */
    public List<UserVO> getUserList(UserQueryForm queryForm) {
        String listCacheKey = buildListCacheKey(queryForm);

        // 1. 查缓存
        List<UserVO> cachedList = (List<UserVO>) localCache.getIfPresent(listCacheKey);
        if (cachedList != null) {
            return cachedList;
        }

        // 2. 查数据库
        List<UserEntity> entityList = userDao.queryList(queryForm);
        List<UserVO> userList = SmartBeanUtil.copyList(entityList, UserVO.class);

        // 3. 缓存结果（只缓存少量数据）
        if (userList.size() <= 100) {
            localCache.put(listCacheKey, userList);
        }
    ...
    public void handleApplicationReady(ApplicationReadyEvent event) {
        ...
        CompletableFuture.runAsync(() -> {
            try {
                List<UserEntity> activeUsers = userDao.queryActiveUsers(1000);
                activeUsers.forEach(user -> {
                    String cacheKey = buildCacheKey(user.getUserId(), INFO_SUFFIX);
                    UserVO userVO = SmartBeanUtil.copy(user, UserVO.class);
                    this.setCacheSync(cacheKey, userVO);
                });
                log.info("缓存预热完成，预热用户数量: {}", activeUsers.size());
            } catch (Exception e) {
                log.error("缓存预热失败", e);
            }
        });
    ...
    private String buildListCacheKey(UserQueryForm queryForm) {
        return CACHE_PREFIX + "list:" + DigestUtils.md5DigestAsHex(
            JSON.toJSONString(queryForm).getBytes());
    }
    ...
    private void setCacheSync(String key, Object value) {
        try {
            localCache.put(key, value);
            redisTemplate.opsForValue().set(key, value, 30, TimeUnit.MINUTES);
        } catch (Exception e) {
...
### 架构规范引用
```markdown
架构设计详见 [架构规范 - 四层架构](./01-核心规范层/架构规范.md#四层架构详细规范)
```

### 数据库规范引用
```markdown
数据库设计详见 [架构规范 - 数据库规范](./01-核心规范层/架构规范.md#数据库架构规范)
```

### 缓存规范引用
```markdown
缓存使用详见 [架构规范 - 缓存架构](./01-核心规范层/架构规范.md#缓存架构规范)
```

### 微服务规范引用
```markdown
微服务设计详见 [架构规范 - 微服务架构](./01-核心规范层/架构规范.md#微服务架构规范)
```
...
**📖 相关权威文档**：
- [编码规范](./编码规范.md) - 编码标准规范
- [API规范](./API规范.md) - API接口设计规范
- [安全规范](./安全规范.md) - 安全相关规范
- [数据规范](./数据规范.md) - 数据存储和处理规范

**📋 版本信息**：
- 本文档整合自SmartAdmin v1、v2、v3版本精华内容
