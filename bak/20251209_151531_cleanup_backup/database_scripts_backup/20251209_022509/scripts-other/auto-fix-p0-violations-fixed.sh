...
# 创建临时模板文件
create_templates() {
    cat > /tmp/microservice_template.md << 'EOF'
## 📋 IOE-DREAM七微服务架构
...
EOF
    ...
    cat > /tmp/four_layer_template.md << 'EOF'
## 🏗️ 四层架构规范
...
EOF
    ...
    cat > /tmp/zero_tolerance_template.md << 'EOF'
...
fix_document() {
    ...
    # 使用临时文件插入架构模板
    sed -i "${insert_line}r /tmp/microservice_template.md" "$file"
    sed -i "${insert_line}a\\ " "$file"
    sed -i "${insert_line}r /tmp/four_layer_template.md" "$file"
    sed -i "${insert_line}a\\ " "$file"
    sed -i "${insert_line}r /tmp/zero_tolerance_template.md" "$file"
...
fix_core_documents() {
    ...
    local core_docs=(
        ...
        "documentation/technical/开发指南.md"
...
main() {
    ...
    echo "1) 快速修复核心文档 (9个高优先级)"
    echo "2) 批量修复所有文档 (400+个)"
    echo "3) 仅验证现有修复"
    echo ""
    echo "自动选择: 快速修复核心文档 (模式1)"
    ...
    # 清理临时文件
    rm -f /tmp/microservice_template.md /tmp/four_layer_template.md /tmp/zero_tolerance_template.md
}
