...
# 执行: powershell -ExecutionPolicy Bypass -File run-all-scans-fixed.ps1
...
扫描项                     | 违规数量 | 状态 | 优先级
--------------------------|---------|------|--------
@Repository注解违规        | $repoViolations | $(if ($repoViolations -eq 0) { "合规" } else { "需要修复" }) | P0
@Autowired注解违规         | $autoViolations | $(if ($autoViolations -eq 0) { "合规" } else { "需要修复" }) | P0
架构边界违规               | $archViolations | $(if ($archViolations -eq 0) { "合规" } else { "需要修复" }) | P0
