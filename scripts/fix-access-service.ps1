...
param(
    [string]$ServicePath = "microservices/ioedream-access-service"
)

Write-Host "🔧 开始修复 access-service LoggerFactory 违规..." -ForegroundColor Green
...
if (-not $remainingViolations) {
    Write-Host "   ✅ access-service LoggerFactory 违规已全部修复！" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host "   bash scripts/scan-logger-violations.sh | grep access-service" -ForegroundColor Gray
} else {
    ...
    $remainingViolations | Select-Object -First 5 | ForEach-Object {
