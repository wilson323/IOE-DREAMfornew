...
Write-Host "Fix All Test Imports (Final Version)" -ForegroundColor Cyan
Write-Host "================================================" -ForegroundColor Cyan
Write-Host ""
...
foreach ($testFile in $testFiles) {
    try {
        ...
        # 尝试多个可能的ServiceImpl路径
        $possiblePaths = @(
            (Join-Path $basePath "src\main\java\$($servicePackage -replace '\.', '\')\${testFileName}Impl.java"),
            (Join-Path $basePath "src\main\java\$($testPackage -replace '\.', '\')\impl\${testFileName}Impl.java"),
